...
#  Source required helper functions if needed
{
    ...
    source "${_dir_src_slurm}/../core/source_helpers.sh" || {
        echo "error($(basename "${BASH_SOURCE[0]}")):" \
            "failed to source '${_dir_src_slurm}/../core/source_helpers.sh'." >&2
...
function set_logs_slurm() {
    ...
    local dir_eo="${4:-}"   # Directory for stderr and stdout log files
...
-----
  set_logs_slurm
    [--help] id_job id_tsk samp dir_eo nam_job
...
----------
  ...
  2  id_tsk : int
    Slurm task ID within the job array.

  3  samp : str
    Sample name associated with the log files.
...
-----
  ...
  - Initial log names follow the form:
      \${dir_eo}/\${nam_job}.\${id_job}-\${id_tsk}.stderr.txt
      \${dir_eo}/\${nam_job}.\${id_job}-\${id_tsk}.stdout.txt
  - Descriptive hard-linked names additionally include '\${samp}'.

Examples
--------
  1. Link one Slurm task's initial logs to sample-descriptive names.
    '''bash
    tmp="\$(mktemp -d)"
    trap 'rm -r -- "\${tmp}"' EXIT
    touch "\${tmp}/align.42-0.stderr.txt" "\${tmp}/align.42-0.stdout.txt"
    set_logs_slurm 42 0 sample_A "\${tmp}" align
    '''

  2. Handle missing initial Slurm logs as an expected failure.
    '''bash
    tmp="\$(mktemp -d)"
    trap 'rm -r -- "\${tmp}"' EXIT
    if ! set_logs_slurm 42 1 sample_B "\${tmp}" align; then
        printf '%s\n' 'missing initial logs rejected as expected'
    fi
    '''
EOM
    )
    ...
    if [[ "${id_job}" =~ ^(-h|--h[e]?lp)$ ]]; then
        ...
        echo_err_func "${FUNCNAME[0]}" \
            "positional argument 4, 'dir_eo', is missing."
        echo >&2
        echo "${show_help}" >&2
        return 1
    elif [[ -z "${nam_job}" ]]; then
    ...
    #  Set log paths
    err_ini="${dir_eo}/${nam_job}.${id_job}-${id_tsk}.stderr.txt"
    out_ini="${dir_eo}/${nam_job}.${id_job}-${id_tsk}.stdout.txt"
    err_dsc="${dir_eo}/${nam_job}.${samp}.${id_job}-${id_tsk}.stderr.txt"
    out_dsc="${dir_eo}/${nam_job}.${samp}.${id_job}-${id_tsk}.stdout.txt"
