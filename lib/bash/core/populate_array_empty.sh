...
----------
  ...
  1  arr_nam : str
    Name of the target array variable (not a reference).

  2  len_tgt : int
    Desired length to populate (must be an integer >= 0).

  3  fill : str
    Value to write into each element (default: "NA").
...
Examples
--------
  1. Populate an unset array with four 'NA' elements.
    '''bash
    unset arr_foo
  ...
  2. Preserve an existing nonempty array despite a new fill request.
    '''bash
    declare -a arr_bar=(existing)
