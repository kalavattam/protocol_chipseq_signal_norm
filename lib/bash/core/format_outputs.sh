...
----------
  1  slurm : bool
    Submit jobs to the Slurm scheduler. Boolean-like string:
...
Returns
-------
  Returns the exit status of the selected 'sed' or 'awk' formatter.
...
Examples
--------
  1. Format a plain wrapper command from one input line.
    '''bash
    printf '%s\n' 'bash bin/execute_compute_signal.sh --method norm' \\
        | format_print_cmd false
    '''

  2. Format an 'sbatch' command and separate its script arguments.
    '''bash
    printf '%s\n' \\
        'sbatch --cpus-per-task 4 bin/execute_compute_signal.sh --method norm' \\
        | format_print_cmd true bin/execute_compute_signal.sh
    '''
EOM
    )
...
Returns
-------
  Returns 0 after printing the banner; otherwise non-zero if output fails.
...
Examples
--------
  1. Print a single-line banner with the default marker and padding.
    '''bash
    print_banner_pretty --text 'Scaling factors complete'
    '''

  2. Wrap trailing text to 30 columns using a percent marker and wider padding.
    '''bash
    print_banner_pretty \\
        --wrap '%' \\
        --pad 2 \\
        --cols 30 \\
        -- \\
        Scaling factor calculation completed for every sample
    '''
EOM
    )
...
Examples
--------
  1. Print an indexed command array with shell escaping.
    '''bash
    arr_cmd=( bash bin/find_files.sh --dir_fnd tests --pattern '*.bam' )
    print_cmd_array arr_cmd
    '''

  2. Confirm that an associative array is rejected.
    '''bash
    declare -A cmd_by_mode=( [bam]='samtools view' )
    if ! print_cmd_array cmd_by_mode; then
        printf '%s\n' 'associative array rejected as expected'
    fi
    '''
EOM
    )
...
----------
  ...
  -it, --indnt, --indent : flag
    Use extra indentation suitable for, e.g., embedding inside a "Command:" block.

Returns
-------
  Returns 0 after printing the formatted command; otherwise 1 for invalid arguments.
...
Examples
--------
  1. Print a command using the default two tokens on its first line.
    '''bash
    print_cmd_pretty bash bin/find_files.sh tests '*.bam'
    '''

  2. Pair flag values and indent a Slurm-oriented command block.
    '''bash
    print_cmd_pretty \\
        --pair_flg \\
        --indent \\
