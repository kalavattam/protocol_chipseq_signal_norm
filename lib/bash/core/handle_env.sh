...
----------
  ...
  1  shl_cur : str
    Optional shell-name hint for Conda hook initialization. This helper is intended for Bash >= 4.4 workflows. If supplied, 'bash' is the primary supported case and 'zsh' is handled only as a limited fallback for hook setup.
...
-----
  ...
  Safely activate a specified Conda environment.

  This helper first initializes the appropriate Conda shell hook for the current shell context, then attempts environment activation, preferring 'mamba' when available and falling back to 'conda' and finally 'source activate'.
...
Examples
--------
  1. Activate an installed project environment.
    '''bash
    _handle_env_activate env_protocol
    '''

  2. Confirm that an absent environment is rejected.
    '''bash
    if ! \\
        _handle_env_activate environment_that_does_not_exist
    then
        printf '%s\n' 'absent environment rejected as expected'
    fi
    '''
EOM
    )
...
-----
  ...
  Ensure that the requested Conda environment is active.
...
Examples
--------
  1. Leave the requested environment unchanged when it is already active.
    '''bash
    CONDA_DEFAULT_ENV=env_protocol handle_env env_protocol
    '''

  2. Confirm that an absent requested environment is rejected.
    '''bash
    (
        if ! \\
            handle_env environment_that_does_not_exist
        then
            printf '%s\n' 'absent environment rejected as expected'
        fi
    )
