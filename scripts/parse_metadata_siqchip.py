...
"""
Deterministically map one siQ-ChIP alignment filename to one metadata row.

The parser only retrieves configured metadata-table values. It does not infer
missing measurements, count alignments, estimate fragment lengths, choose siQ
equations, or calculate scaling coefficients.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "-a", "--alignment", "--aln", "--aln-file", "--aln_file",
        "-b", "--bam",
        "-cr", "--cram",
        dest="alignment",
    )
    ...
    parser.add_argument(
        "-sp", "--skp_pfx",
        default=None,
        help="Comma-separated metadata line prefixes to skip.",
    )
