...
Keyword arguments:
  ...
  -do, --dir_out  <dir>
    Directory to save queryname-sorted alignment files.
...
Notes:
  ...
  - This is a submit-wrapper script: it supports serial/local iteration and
    Slurm-array task execution, but it does not submit Slurm jobs itself.
EOM
}
...
function run_job() {
    ...
    if ! \
        qsort_file_alignments \
            ...
            "${log_err}"
    then
        echo_err "failed to sort file: '${infile}'. See log: '${log_err}'."
        return 1
    fi
