...
from help_style import sections
...
class Review:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        value["signature"] = "sha256:" + hashlib.sha256(payload.encode()).hexdigest()
        return value
...
def positional_kind(value: str) -> str:
    ...
    if stripped.startswith(("${", "$")):
        return "expansion"
    if "/" in stripped or re.search(r"\.(?:bam|cram|bed|bdg|gz|fq|fastq|txt)$", stripped):
        return "path"
    return "value"
...
def option_value_shape(value: str) -> str:
    ...
    if "," in stripped:
        delimiters += "comma_"
    extension = re.search(r"\.(bam|cram|fastq|fq|bdg|bed)(?:\.gz)?(?:[,;]|$)", stripped)
    suffix = f"{extension.group(1)}" if extension else positional_kind(stripped)
...
def invocation_facts(
    ...
    while index < len(tail):
        ...
        if OPTION.fullmatch(token):
            ...
            if index + 1 < len(tail) and not tail[index + 1].startswith("-"):
                value_shapes.append((token, option_value_shape(tail[index + 1])))
                index += 2
            else:
                index += 1
            continue
    ...
    setup_shape = tuple(
        ...
        for shape, pattern in (
            ...
            ("function_definition", r"^\s*(?:function\s+)?[A-Za-z_][A-Za-z0-9_]*\s*\(\)\s*\{"),
    ...
    skip_failure_shape = tuple(
        ...
        for shape, pattern in (
            ...
            ("expected_failure", r"\b(?:if|elif)\s+!\s+|(?:^|[;&|]\s*)!\s+|\|\|"),
...
def analyze_help_document(
    ...
    *,
    ...
    example_sections = [section for section in parsed if section.name == "Examples"]
    ...
    if example_sections:
        ...
        observed_numbers = [int(match.group("number")) for _, _, match in starts if match]
        ...
        for position, (start, source_line, match) in enumerate(starts):
            ...
            entry_lines = body[start + 1 : end]
            openings = [
                index for index, (_, line) in enumerate(entry_lines) if line == "    '''bash"
            ]
            closings = [
                index for index, (_, line) in enumerate(entry_lines) if line == "    '''"
            ]
            if len(openings) != 1 or len(closings) != 1:
                strict_findings.append(
                    _finding(
                        ...
                        source_line,
                        owner,
                        "each numbered entry requires exactly one '''bash ... ''' block",
                        line_offset,
                    )
                )
                continue
            ...
            if any(line and not line.startswith("    ") for _, line in code_rows):
                strict_findings.append(
                    _finding(
                        ...
                        source_line,
                        owner,
                        "example code must use the established four-space indentation",
                        line_offset,
                    )
                )
            code = tuple(line[4:] if line.startswith("    ") else line for _, line in code_rows)
            ...
            for alias in sorted(observed_options & hidden_aliases):
                strict_findings.append(
                    _finding(
                        ...
                        source_line,
                        owner,
                        f"hidden compatibility alias '{alias}' may not appear in examples",
                        line_offset,
                    )
                )
            ...
            for alias in sorted((observed_options & accepted_aliases) - public_aliases):
                if alias not in hidden_aliases:
                    strict_findings.append(
                        _finding(
                            ...
                            source_line,
                            owner,
                            f"accepted nonpublic alias '{alias}' may not appear in examples",
                            line_offset,
                        )
                    )
    ...
    for index, first in enumerate(examples_parsed):
        for second in examples_parsed[index + 1 :]:
            if first.normalized_invocation == second.normalized_invocation:
                strict_findings.append(
                    _finding(
                        ...
                        owner,
                        f"examples {first.number} and {second.number} repeat one invocation",
                        line_offset,
                    )
                )
            if first.signature == second.signature:
                strict_findings.append(
                    _finding(
                        ...
                        second.line - line_offset,
                        owner,
                        f"examples {first.number} and {second.number} have identical normalized signatures",
                        line_offset,
                    )
                )
            ...
            explicitly_distinct_outcomes = (
                same_branch_evidence
                and first.expected_outcome_shape
                != second.expected_outcome_shape
            )
            if (
                ...
                reviews.append(
                    Review(
                        ...
                        "examples share option and positional shape; confirm that branch or domain meaning is materially distinct",
                    )
                )
...
def classify_wrapper_source(path: str, text: str) -> Ownership:
    ...
    stem = Path(path).stem
    if re.search(r"--details", text) and re.search(rf"\bdetail_{re.escape(stem)}\b", text):
        surface = "--details"
        if "--all_help" in text or "--all[_-]h[e]?lp" in text:
            surface += "; --all_help renders short plus full help"
        return Ownership(path, "details_full_document", f"detail_{stem}", surface)
    help_owner = f"help_{stem}"
    if re.search(rf"\b{re.escape(help_owner)}\b", text):
        return Ownership(path, "default_help_full_document", help_owner, "--help")
...
def undispositioned_review_findings(
    ...
    return [
        Finding(
            ...
            review.line,
            identity,
            f"semantic candidate {review.as_dict()['signature']} requires an explicit disposition",
        )
...
def top_level_shell_paths(root: Path) -> list[Path]:
    ...
    paths = [*(root / "bin").glob("*.sh"), *(root / "install/scripts").glob("*.sh")]
...
def centralized_script_help_units(
    ...
    for source in top_level_shell_paths(root):
        ...
        classified = (
            classify_wrapper_source(relative, source.read_text(encoding="utf-8"))
            if source.name.startswith(("execute_", "submit_"))
            else None
        )
        if classified and classified.classification == "compatibility_delegate":
            continue
        ...
        heredoc = next((item for item in docs if item.owner == preferred), None)
...
def script_help_units(root: Path) -> dict[str, tuple[Path, Path, Heredoc]]:
    ...
    return {**centralized_script_help_units(root), **inline_script_help_units(root)}
...
def function_help_units(root: Path) -> dict[str, tuple[Path, Heredoc]]:
    ...
    for path in shell_help_paths(root):
        ...
        for heredoc in extract_help_heredocs(path.read_text(encoding="utf-8")):
            if heredoc.owner == "<file>" or (
                relative, heredoc.owner, heredoc.start_line
            ) in centralized:
                continue
...
def repository_crosswalk(root: Path) -> dict[str, object]:
    ...
    categories = [
        category("all_parsed_help_bearing_source_units", "Every bounded sectioned help heredoc parsed from maintained shell sources.", parsed, "Universe row; owner and non-owner categories below partition or overlap it explicitly.", False),
        category("shell_function_heredoc_owners", "Parsed heredocs lexically owned by shell functions after centralized script help and shared fragments are removed.", list(functions), "Disjoint from top-level script owners and shared heredocs.", True),
        category("top_level_shell_script_help_owners", "Maintained top-level script interfaces with one inline or centralized full-help document.", list(scripts), "Disjoint owner class; centralized functions delegate ownership to these script identities.", True),
        category("centralized_help_functions", "Functions in lib/bash/help that render a top-level script's help.", [f"{help.relative_to(root)}::{doc.owner}" for _, help, doc in centralized.values()], "Overlaps parsed units but delegates ownership to top_level_shell_script_help_owners.", False),
        category("execute_submit_rendered_wrapper_surfaces", "Canonical execute/submit wrapper full-help surfaces classified from current parser dispatch.", [f"{item.path}::{item.full_help_owner}" for item in wrappers], "Subset of top_level_shell_script_help_owners; compatibility delegators are separate.", True),
        category("compatibility_delegators", "Execute/submit shims that exec one canonical wrapper and own no separate full-help document.", [f"{item.path}->{item.target}" for item in delegators], "Delegate to the target wrapper and do not add a second owner.", False),
        category("shared_heredoc_nonowners", "Parsed section fragments outside a function that are interpolated into one or more owned help documents.", shared, "Overlap parsed universe only; not independently callable help owners.", False),
        category("excluded_test_or_synthetic_fixtures", "Generated outputs and Python string fixtures excluded by bounded repository discovery.", [], "Outside the parsed source universe; real tests shell functions remain governed.", False),
    ]
    return {
        "schema_version": 1,
...
def wrapper_paths(root: Path) -> list[Path]:
    ...
    return sorted(
        [*(root / "bin").glob("execute_*.sh"), *(root / "bin").glob("submit_*.sh")]
    )


def wrapper_aliases(root: Path, wrapper: Path) -> tuple[set[str], set[str], set[str]]:
    ...
    if re.search(r"-h\|--h\[e\]\?lp|-h\|--hlp\|--help", text):
        accepted.update({"-h", "--hlp", "--help"})
    hidden = {alias for alias in accepted if hidden_alias(alias, tuple(sorted(accepted)))}
    help_path = root / "lib/bash/help" / f"help_{wrapper.name}"
    public = set()
    if help_path.is_file():
        public = {alias for row in parameter_rows(help_path.read_text()) for alias in row.aliases}
...
def script_aliases(
    ...
    text = source.read_text(encoding="utf-8")
    chunks = alias_chunks(
        text, {"parse_args", "resolve_dir_scr", "check_args_light", "main"}
    )
    if not chunks:
...
def scan_repository(root: Path) -> RepositoryResult:
    ...
    for item in ownership:
        ...
        if canonical.classification == "no_valid_full_help_owner":
            ...
            inventory.append({**item.as_dict(), "status": "ownership_violation"})
            continue
        ...
        if canonical_name not in analyzed_canonical:
            help_path = (
                root
                / "lib/bash/help"
                / f"help_{canonical_name}"
            )
            docs = extract_help_heredocs(help_path.read_text(encoding="utf-8"))
            heredoc = next(
                (doc for doc in docs if doc.owner == canonical.full_help_owner),
                None,
            )
            if heredoc is None:
                analysis = Analysis(
                ...
                accepted, public, hidden = wrapper_aliases(root, canonical_wrapper)
                ...
                if canonical.classification == "details_full_document":
                    ...
                    if not short_help_advertises_details(
                        help_path.read_text(encoding="utf-8"), short_owner
                    ):
                        analysis = Analysis(
                            [
                                *analysis.findings,
                                Finding(
                                    ...
                                    1,
                                    short_owner,
                                    "concise --help must advertise the valid --details full-help surface",
                                ),
                            ],
                            analysis.reviews,
                            analysis.examples,
        ...
        inventory.append(
            {
                **item.as_dict(),
                ...
                "examples": [example.as_dict() for example in analysis.examples],
            }
        )
    ...
    for identity, (source, help_path, heredoc) in sorted(script_units.items()):
        ...
        if canonical is None:
            ...
            pending = undispositioned_review_findings(identity, canonical.reviews)
            if pending:
                canonical = Analysis(
        ...
        inventory.append(
            {
                ...
                "examples": [example.as_dict() for example in canonical.examples],
            }
        )
    ...
    for identity in sorted(expected - observed):
        findings.append(
            Finding(
                ...
                1,
                identity,
                "source-derived help owner is absent from the strict inventory",
            )
        )
...
def compliance_summary(result: RepositoryResult) -> dict[str, object]:
    ...
    functions = [row for row in result.inventory if row.get("kind") == "function"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "script help owners: "
        f"{summary['script_green']}/{summary['script_total']} strict-green"
    )
    print(
        "function help owners: "
        f"{summary['function_green']}/{summary['function_total']} strict-green"
    )
