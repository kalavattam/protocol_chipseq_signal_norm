...
def direct_tests(path: str, role: str) -> tuple[str, ...]:
    ...
    if path.startswith("src/protocol_chipseq_signal_norm/cli/") and role == "entry_point":
        return ("tests/contract/repository/test_python_startup.sh",)
    if path.startswith("src/protocol_chipseq_signal_norm/"):
        return ("tests/unit", "tests/contract/repository/test_python_startup.sh")
...
def local_guard(text: str) -> str:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Assert):
            continue
        expression = ast.get_source_segment(text, node.test) or ast.unparse(node.test)
        ...
        if message != CANONICAL_MESSAGE:
            findings.append(
                Finding(
                    ...
                    path,
                    node.lineno,
                    f"facet=guard; local guard message must use '{CANONICAL_MESSAGE}'",
                )
            )
    return findings
...
def scan_repository(root: Path) -> tuple[list[Finding], list[PythonSource]]:
    ...
    if not is_supported_version(sys.version_info[:2]):
        findings.append(
            Finding(
                ...
                "<interpreter>",
                1,
                f"facet=interpreter; repository checks require {POLICY_LABEL}; observed {sys.version.split()[0]}",
            )
        )
    ...
    environment_text = (
        environment.read_text(encoding="utf-8") if environment.is_file() else ""
    )
    environment_version = environment_python_version(environment_text)
    if (
        environment_version is None
        or not is_supported_version(environment_version)
    ):
        findings.append(
            Finding(
                RULE_ID,
                ...
                1,
                "facet=environment; authoritative environment must select Python 3.11 or newer",
            )
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "Python policy: "
        f"{len(inventory)} maintained file(s), "
