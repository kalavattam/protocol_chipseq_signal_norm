...
def resolve_env_bash(environment: str = "env_protocol") -> BashInterpreter:
    ...
    if len(rows) < 2:
        raise RuntimeError("could not resolve Bash path and version from env_protocol")
    path = Path(rows[0]).resolve()
    match = re.match(r"(?P<major>\d+)\.(?P<minor>\d+)\.(?P<patch>\d+)", rows[1])
    if not path.is_absolute() or match is None:
        raise RuntimeError("env_protocol returned an invalid Bash path or version")
