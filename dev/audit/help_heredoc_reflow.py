...
class Finding:
    ...
    def format(self) -> str:
        ...
        return (
            ...
            f"physical={self.offending_lines[0]!r} | {self.offending_lines[1]!r}; "
...
def is_help_body(lines: list[tuple[int, str]], owner: str, start_source: str) -> bool:
    ...
    context_named = owner.startswith(("help_", "show_help", "detail_")) or "help" in start_source.lower()
...
def _example_section_rows(
    ...
    for index, (_, line) in enumerate(body[:-1]):
        name = line.strip()
        if name in SECTION_NAMES and UNDERLINE.fullmatch(body[index + 1][1].strip()):
...
def extract_rendered_examples_text(text: str) -> str:
    ...
    for index, line in enumerate(lines[:-1]):
        name = line.strip()
        if name in SECTION_NAMES and UNDERLINE.fullmatch(lines[index + 1].strip()):
            starts.append((index, name))
    examples: list[str] = []
    for position, (index, name) in enumerate(starts):
        if name != "Examples":
    ...
    if len(examples) != 1:
        raise ValueError(
            f"expected exactly one rendered Examples section; found {len(examples)}"
        )
    return examples[0]
...
def _complex_reason(code: list[tuple[int, str]]) -> str | None:
    ...
    for _, line in code:
        ...
        if COMPLEX_HEAD.match(stripped):
            return "shell control-flow syntax has separate indentation semantics"
        ...
        if ASSIGNMENT.match(stripped):
            ...
            return "assignment/setup snippet is not one ordinary CLI invocation"
        if "<<" in stripped:
        ...
        if re.search(r"(?:^|\s)(?:\||\|\||&&)(?:\s|$)", stripped):
            return "structured pipeline or command list is deliberately excluded"
        if re.search(r"(?:^|\s)(?:>>?|<)(?:\s|$)", stripped):
    ...
    *,
    ...
    inventory.append(
        ExampleCommandInventory(
            ...
            reason="bounded ordinary command with physical argument continuations",
    ...
    for index, (number, line) in enumerate(command):
        ...
        if COLLAPSED_CONTINUATION.search(line):
            _finding(
                ...
                number,
                owner,
                "continuation and following option are collapsed on one physical line",
            )
            if rendered:
                _finding(
                    findings,
                    ...
                    number,
                    owner,
                    "rendered command did not preserve the required physical line boundary",
                )
        if trailing:
            _finding(
                findings,
        ...
        if is_final:
            if slash_count:
                _finding(
                    ...
                    number,
                    owner,
                    "final command line must not end with a continuation backslash",
                )
            continue
        if slash_count != expected_slashes:
            ...
            _finding(
                ...
                number,
                owner,
                f"non-final {representation} line requires exactly {expected_slashes} terminal backslash(es); observed {slash_count}",
            )
    ...
    for number, line in command[1:]:
        ...
        if observed != base_indent + 4:
            _finding(
                ...
                number,
                owner,
                f"continuation indentation must be exactly four spaces beyond base {base_indent}; observed {observed}",
            )
    ...
    for section in _example_section_rows(heredoc):
        ...
        if not starts and any(line.strip() for _, line in section):
            ...
            _finding(
                ...
                number,
                heredoc.owner,
                "recognized Examples content has no numbered entry/fence structure",
            )
            inventory.append(
                ExampleCommandInventory(
                    path,
        ...
        for position, start in enumerate(starts):
            ...
            if position and section[start - 1][1].strip():
                rule = RULE_EXAMPLE_RENDERED_STRUCTURE if rendered else RULE_EXAMPLE_FENCE
                _finding(
                    findings,
                    rule,
                    path,
                    example_line,
                    heredoc.owner,
                    "numbered Examples entries require one separating blank line",
                )
            malformed_fences = [
                (number, line)
            ...
            valid_pair = (
                ...
                and _leading_whitespace(openings[0][2])
                == _leading_whitespace(closings[0][2])
            )
            if malformed_fences or not valid_pair:
                ...
                _finding(
                    ...
                    line,
                    heredoc.owner,
                    "each recognized Examples entry requires one literal '''bash ... ''' fence pair",
                )
                inventory.append(
                    ExampleCommandInventory(
                        path,
                        ...
                        line,
                        "malformed_or_ambiguous_block",
                        "missing, changed, duplicated, or unterminated literal fence",
                        representation,
                        heredoc.quoted,
                    )
                )
            ...
            if not commands:
                inventory.append(
                    ExampleCommandInventory(
                        ...
                        closing[1],
                        "malformed_or_ambiguous_block",
                        "no classifiable command remains after comments and blanks",
                        representation,
                        heredoc.quoted,
                    )
                )
...
def structured_lines(heredoc: Heredoc) -> set[int]:
    ...
    for index, (number, line) in enumerate(lines):
        ...
        if not stripped:
            ...
            list_indent = None
            continue
        if stripped in SECTION_NAMES and UNDERLINE.fullmatch(next_line.strip()):
            section = stripped
            structured.add(number)
            continue
        ...
        if "|" in line or "\t" in line or re.search(r"\S\s{2,}\S", stripped):
            structured.add(number)
            continue
        if re.match(r"^(?:[$>]\s*)?(?:bash|sh|python\d*|conda|git|printf|echo|cat)\b", stripped):
            structured.add(number)
            continue
        ...
        for (first_number, first), (second_number, second) in pairwise(body):
            ...
            findings.append(
                Finding(
                    ...
                    rendered_prose=normalized(f"{first.strip()} {second.strip()}"),
                )
            )
    return findings


def scan_repository(root: Path, paths: Iterable[str] | None = None) -> list[Finding]:
    ...
    return [finding for path in sorted(set(selected)) for finding in findings_for_path(root, path)]
...
def example_inventory_summary(
    ...
    checked = [
        row for row in audit.inventory if row.classification == "checked_simple_command"
    ]
    excluded = [
        row
...
def fix_source_example_commands(
        ...
        sorted(
            {
                ...
                for finding in audit.findings
                if finding.rule_id in {RULE_EXAMPLE_COLLAPSED, RULE_EXAMPLE_FENCE}
            }
            | {
    ...
    for row in audit.inventory:
        ...
        for position, line in enumerate(original):
            content = line.lstrip(" \t") if position else line[len(base_whitespace) :]
            content = re.sub(r"\\+[ \t]*$", "", content).rstrip(" \t")
            indentation_prefix = (
                base_whitespace if position == 0 else " " * (len(base_whitespace) + 4)
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.extract_rendered_examples is not None:
        try:
            ...
            print(f"{RULE_EXAMPLE_FENCE}: {args.extract_rendered_examples}: {exc}")
            return 1
        return 0
    if args.fix_example_commands:
        ...
        for path in selected:
            ...
            if after != before:
                ...
                print(f"HELP.EXAMPLES.COMMAND.FIX: {path}: normalized {count} command(s)")
        if refused:
            for item in refused:
                print(f"HELP.EXAMPLES.COMMAND.FIX_REFUSED: {item}")
            return 1
    prose_findings = [] if args.rendered else scan_repository(args.root, args.paths or None)
            ...
            json.dumps(
                {
                    ...
                    "inventory": [row.as_dict() for row in example_audit.inventory],
                    "findings": [dataclasses.asdict(row) for row in example_audit.findings],
                },
                indent=2,
                sort_keys=True,
            )
    ...
    for finding in example_audit.findings:
        print(finding.format())
    if prose_findings:
        print(f"{RULE_ID}: {len(prose_findings)} newly introduced or modified violation(s)")
    else:
        print(f"{RULE_ID}: pass (zero newly introduced or modified violations)")
    summary = example_inventory_summary(example_audit)
    if not args.prose_only:
        print(
            "HELP.EXAMPLES.COMMAND: "
            f"{summary['applicable_simple_commands']} applicable; "
            f"{summary['compliant_simple_commands']} compliant; "
            f"{summary['deliberately_excluded_complex_snippets']} complex excluded; "
            f"{summary['ambiguous_snippets']} ambiguous; "
