...
OWNER_CLASSIFICATION = re.compile(
    r"`(deterministic|advisory|semantic-only)`"
)
EMITTED_RULE_ID = re.compile(
...
def _owner_documents(
    ...
    if index.is_file():
        relatives = INDEX_STANDARD_LINK.findall(index.read_text(encoding="utf-8"))
        ...
        for relative in dict.fromkeys(relatives):
            if relative == "README.md" or Path(relative).name.startswith("bak."):
                continue
                ...
                findings.append(
                    {
                        ...
                        "rule_id": identifiers[0],
                        "detail": f"{relative}: {heading}: {classification_text}",
                    }
                )
                continue
...
def _registry_manifest(root: Path) -> dict[str, object]:
        ...
        findings.append(
            {
                ...
                "rule_id": "<registry>",
                "detail": f"expected schema_version 2, found {data.get('schema_version')}",
            }
        )
    ...
    for rule_id, owners in sorted(inventory.items()):
        ...
        if owner.owner_classification == "semantic-only":
            review_text = f"{owner.automation} {owner.semantic_remainder}".lower()
            if not any(
                token in review_text
                for token in ("review", "decide", "determine", "classify", "choose")
            ):
                findings.append(
                    {
                        "kind": "semantic_only_without_review_procedure",
                        "rule_id": rule_id,
                        "detail": f"{owner.normative_document}: {owner.normative_section}",
                    }
                )
            ...
            findings.append(
                {
                    ...
                    "rule_id": rule_id,
                    "detail": "deterministic owner has no executable registry entry",
                }
            )
            ...
            findings.append(
                {
                    ...
                    "rule_id": rule_id,
                    "detail": "owner names executable evidence but has no registry entry",
                }
            )
    ...
    for rule in rules:
        ...
        elif section not in HEADING.findall(document.read_text(encoding="utf-8")):
            findings.append(
                {
        ...
        elif len(owners) == 1 and owner_classification != owners[0].owner_classification:
            findings.append(
                {
        ...
        elif coverage_relation in {"subset", "independent"} and remaining_scope == "None":
            findings.append(
                {
            ...
            findings.append(
                {
                    ...
                    "rule_id": rule_id,
                    "detail": f"role={execution_role}; relation={coverage_relation}",
                }
            )
        if execution_role == "evidence_producer" and coverage_relation == "exact":
            findings.append(
                {
        ...
        if owner_classification == "semantic-only" and coverage_relation != "independent":
            findings.append(
                {
        ...
        if execution_role == "checker" and (
            not fixture or (_is_path(fixture) and not (root / fixture).exists())
        ):
            findings.append(
                {
        ...
        owners_output.append(
            {
                **dataclasses.asdict(owner),
                "coverage_status": "registered" if executions else "review_only",
    ...
    partition = {
        ...
        "review_only_owner_total": sum(
            owner["coverage_status"] == "review_only" for owner in owners_output
        ),
