...
from dev.audit.help_aliases import alias_chunks
from dev.audit.help_examples import function_help_units, script_help_units
from dev.audit.help_heredoc_reflow import Heredoc, extract_help_heredocs
from dev.audit.help_style import (
    active_top_level_sections,
    exact_callable_message,
    load_registry,
)
from dev.audit.shell_validation import MINIMUM_BASH
from dev.audit.source_policy import classify_source
...
RUNTIME_PARENT_MESSAGE = (
    "Runtime requirements must occur inside the active top-level Notes section"
)

RUNTIME_LABEL = re.compile(r"^(?P<indent> *)Runtime requirements:\s*$")
ITEM = re.compile(r"^(?P<indent> *)(?P<marker>- )?(?P<text>\S.*)$")
CONDITION = re.compile(
    r"^(?P<requirement>.+?) \(when (?P<condition>[^()]+)\)$",
)
VERSION = re.compile(
...
INTERNAL = re.compile(
    r"(?:^|\s)(?:bin/|lib/bash/|src/|install/scripts/|tests/|"
...
PYTHON_EXECUTABLE = re.compile(
    r"^(?P<name>python3?|Python)(?:\s+(?P<rest>.*))?$",
)
...
SETTLED_OWNER_REQUIREMENTS: dict[str, tuple[str, ...]] = {
    ...
    (
        "lib/bash/workflows/filter_alignment.sh::_validate_args_filter_alignme"
        "nt"
    ): (
        "bash >= 4.4",
        "dirname",
    ...
    "lib/bash/workflows/filter_alignment.sh::_finalize_alignment_filter": (
        ...
        (
            "Reference FASTA and required index (when 'chk_chr' is true and "
            "processing CRAM)"
        ),
        "samtools",
    ...
    (
        "lib/bash/help/help_execute_compute_signal.sh::help_execute_compute_si"
        "gnal"
    ): (
        "bash >= 4.4",
    ...
    (
        "lib/bash/workflows/process_alignments.sh::convert_alignments_bed_pyth"
        "on"
    ): (
        "bash >= 4.4",
        "python >= 3.11",
    ...
    "lib/bash/workflows/calculate_scaling_factor.sh::_resolve_typ_fil": (
        ...
        (
            "Reference FASTA and required index (when 'pref' is 'auto' or "
            "empty and processing CRAM)"
        ),
    ...
    "lib/bash/workflows/calculate_scaling_factor.sh::_calculate_frag_avg": (
        ...
        (
            "Reference FASTA and required index (when deriving fragment "
            "length from CRAM)"
        ),
        "samtools",
    ),
    ...
    "lib/bash/workflows/calculate_scaling_factor.sh::process_samp_siq": (
        ...
        "Reference FASTA and required index (when processing CRAM)",
        (
            "samtools (when alignment depth, fragment length, or layout must "
            "be derived)"
        ),
    ),
    ...
    "bin/submit_compute_signal.sh::run_comp_sig": (
        ...
        "dirname",
        (
            "Input BAM or CRAM index (when the selected signal engine uses "
            "indexed access)"
        ),
        "python >= 3.11",
        "Reference FASTA and required index (when processing CRAM)",
    ...
    "tests/support/test_helpers.sh::assert_cram_count": (
        (
            "A compatible Conda environment providing samtools (when "
            "'samtools' is unavailable)"
        ),
        "bash >= 4.4",
        "conda (when 'samtools' is unavailable)",
        "dirname",
...
def owner_units(root: Path) -> dict[str, tuple[str, Path, Path, Heredoc]]:
    ...
    for identity, (source, documentation, heredoc) in script_help_units(
        root,
    ).items():
...
def runtime_entries(
    ...
    ----------
    heredoc : Heredoc
        Parsed rendered help rows and section boundaries.

    Returns
    -------
    label, entries, grammar : tuple[
        int | None, list[tuple[int, str]], list[str]
    ]
        Runtime label line, requirement rows, and structural grammar messages.
    """
    ...
    grammar: list[str] = []
    active_sections = active_top_level_sections(heredoc)
    ...
    for index, (number, line) in enumerate(rows):
        ...
        if active_sections.get(number) != "Notes":
            grammar.append(RUNTIME_PARENT_MESSAGE)
        ...
        for content_number, content in rows[index + 1 :]:
            ...
            category_continuation = text.startswith(
                (
                    "+ ",
                    "- Programs",
                    "- Environment",
            ...
            if text.endswith(":") or category_continuation:
                grammar.append(
                    f"line {content_number} uses a category or continuation",
                )

                continue

            entries.append(
                (content_number, (match.group("marker") or "") + text),
            )
...
def normalize_runtime_wording(value: str) -> str:
    ...
    ----------
    value : str
        Parsed runtime-requirement entry.

    Returns
    -------
    normalized : str
        Canonical capitalization, quoting, provider, and condition wording.
    """
    ...
    conda_environment_heads = {
        "A compatible Conda environment",
        "A compatible Conda environment providing parallel",
        "A compatible Conda environment providing the listed tools",
    }
    inactive_environment_conditions = {
        "the requested environment is not active",
        "no non-base project environment is active",
    }
    is_conda_environment = head in conda_environment_heads
    is_inactive_condition = condition in inactive_environment_conditions

    if is_conda_environment and is_inactive_condition:
        condition = None
    ...
    if head == "python":
        ...
        head = (
            "python >= 3.11 or python3 >= 3.11, each with loopback socket "
            "support"
        )
...
def normalize_runtime_contract(entries: list[str]) -> list[str]:
    ...
    normalized: list[str] = []
    add_conda_activation = any(
        entry.startswith(OLD_ENVIRONMENT) for entry in entries
    )
    manager_alternative = (
        "mamba" in entries and "conda (when mamba is unavailable)" in entries
    )

    for entry in entries:
        is_manager_alternative = entry in {
            "mamba",
            "conda (when mamba is unavailable)",
        }

        if manager_alternative and is_manager_alternative:
            continue
...
def quoted_ranges(text: str) -> list[tuple[int, int]]:
    ...
    return [
        (match.start(), match.end()) for match in SINGLE_QUOTED.finditer(text)
    ]
...
def is_executable_requirement(value: str, callables: set[str]) -> bool:
    ...
    return (
        bool(provider_tokens)
        and re.search(r"\b(?:or|and)\b", head) is not None
    )
...
def python_minimum_message(value: str) -> str | None:
    ...
    if CALLER_PYTHON.match(head):
        if re.search(r"\bPython interpreter >= 3\.11\b", head) is None:
            return (
                "caller-supplied interpreters must require 'Python "
                "interpreter >= 3.11'"
            )
    ...
    if head.startswith("caller-supplied Python interpreter"):
        return (
            "caller-supplied interpreter prose must begin with "
            "'Caller-supplied'"
        )
    ...
    if head.startswith("Python"):
        return (
            "an exact python executable requirement must use lowercase "
            "callable spelling"
        )
    ...
    expected = {
        provider.split()[0] + f" >= {PYTHON_MINIMUM}" for provider in providers
    }
...
def inline_api_message(
    ...
    if "`" in condition:
        return (
            "Runtime conditions use straight single quotes, not Markdown "
            "backticks"
        )

    if "-'" in condition or re.search(
        r"'--[^']+'\s+[A-Za-z0-9_.-]+\s+is\b",
        condition,
    ):
    ...
    for pattern in (OPTION_TOKEN, SNAKE_API_TOKEN):
        for match in pattern.finditer(condition):
            if not position_is_quoted(match.start(), ranges):
                return (
                    f"quote API expression '{match.group(0)}' with straight "
                    f"single quotes"
                )
    ...
    for match in re.finditer(r"\b[A-Za-z][A-Za-z0-9_.-]*\b", condition):
        if match.group(0) in known and not position_is_quoted(
            match.start(),
            ranges,
        ):
            return (
...
def runtime_policy_messages(
    ...
    if value.endswith("."):
        messages.append(
            (RULE_PUNCTUATION, "Runtime entries do not end with a period"),
        )
    ...
    if CONDA_ENVIRONMENT.search(value):
        messages.append(
            (
                ...
                (
                    "environment resources use 'A compatible Conda "
                    "environment' wording"
                ),
            ),
        )
        ...
        messages.append(
            (
                RULE_CONDA_ENVIRONMENT,
                (
                    "Conda environment resources begin with capitalized 'A "
                    "compatible'"
                ),
            ),
        )
    ...
    if REDUNDANT_COMPOSITE.search(value):
        messages.append(
            (
                ...
                (
                    "separate the Conda environment, activation executable, "
                    "and tool entries"
                ),
            ),
        )
    ...
    if (
        ...
        messages.append(
            (
                RULE_CAPITALIZATION,
                (
                    "prose/resource requirements begin with sentence-style "
                    "capitalization"
                ),
            ),
        )
...
def evidence_for_owner(
    ...
    ----------
    identity : str
        Canonical help-owner identity.
    source : Path
        Shell source that implements the owner.
    kind : str
        Script or function ownership class.
    root : Path
        Repository root used to render source-relative evidence paths.

    Returns
    -------
    evidence : list[Evidence]
        Literal invocation, source, condition, and provider observations.
    """
            ...
            if (
                ...
                evidence.append(
                    Evidence(
                        ...
                        str(source.relative_to(root)),
                        number,
                        "callable_alternative",
                        "sha256sum_or_shasum",
                        f"command -v {name}",
                        "direct",
                        "conditional",
                        "when --dry_run is not specified",
                        True,
                        "settled",
                    ),
                )
            elif (
                identity == REGION_BEDGRAPH_CHECK
                and name in DECOMPRESSION_PROVIDERS
            ):
                evidence.append(
                    Evidence(
                        identity,
                        ...
                        "direct",
                        "conditional",
                        "when processing a gzipped bedGraph",
                        True,
                        "settled",
                    ),
                )
                ...
                evidence.append(
                    Evidence(
                        ...
                        str(source.relative_to(root)),
                        number,
                        "optional_reuse_target",
                        name,
                        f"command -v {name}",
                        "direct",
                        "conditional",
                        "when --if_exists reuse is specified",
                        False,
                        "settled",
                    ),
                )
                ...
                evidence.append(
                    Evidence(
                        ...
                        "available in PATH",
                        True,
                        "high" if category == "callable" else "low",
                    ),
                )
...
def candidate(
    ...
    return {
        ...
        "rule_id": "RUNTIME.SEMANTIC.UNKNOWN_CALLABLE",
        "owner": owner,
        "uncertainty_class": "literal-command-classification",
        "evidence": evidence.as_dict(),
        "current_text": documented,
        "disposition_options": [
            "document as callable",
            "classify as shell/local syntax",
            "exclude as internal",
        ],
        "preferred_disposition": (
            "classify before asserting a public requirement"
        ),
    }


def _runtime_shape_findings(
    identity: str,
    path: str,
    line: int,
    entries: list[str],
    ...
    """
    Normalize entries and report cardinality or ordering problems.
    """

    findings: list[Finding] = []
    markers = [text.startswith("- ") for text in entries]

    if len(entries) == 1 and markers and markers[0]:
        findings.append(
            Finding(
                RULE_CARDINALITY,
                identity,
                path,
                line,
                "one requirement must be unbulleted",
            ),
        )
    ...
    if len(entries) > 1 and not all(markers):
        findings.append(
            Finding(
                ...
                path,
                line,
                "two or more requirements must use '-' bullets",
            ),
        )

    normalized = [
        text[2:] if text.startswith("- ") else text for text in entries
    ]
    ...
    if len(normalized) > 1 and normalized != expected_order:
        findings.append(
            Finding(
                ...
                line,
                (
                    "multi-item Runtime requirements must be sorted "
                    "case-insensitively by complete displayed text"
                ),
...
def _runtime_coverage_findings(
    ...
    """
    Report requirements established by repository-owned evidence.
    """

    findings: list[Finding] = []
    minimum_bash = f"bash >= {MINIMUM_BASH[0]}.{MINIMUM_BASH[1]}"

    if minimum_bash not in normalized:
        findings.append(
            Finding(
                RULE_MISSING,
                identity,
                path,
                line,
                "missing inherited requirement 'bash >= 4.4'",
                ("bash >= 4.4",),
                tuple(normalized),
            ),
        )
        ...
        if requirement not in normalized:
            findings.append(
                Finding(
                    ...
                    path,
                    line,
                    f"missing source-reviewed requirement '{requirement}'",
                    (requirement,),
                    tuple(normalized),
                ),
            )
    ...
    if (
        "sha256sum_or_shasum" in evidence_keys
        and CHECKSUM_REQUIREMENT not in normalized
    ):
        findings.append(
            Finding(
                RULE_MISSING,
                ...
                path,
                line,
                "missing alternative SHA-256 provider requirement",
                (CHECKSUM_REQUIREMENT,),
                tuple(normalized),
            ),
        )
    ...
    if (
        "gzip_or_zcat" in evidence_keys
        and DECOMPRESSION_REQUIREMENT not in normalized
    ):
        findings.append(
            Finding(
                RULE_MISSING,
                ...
                line,
                (
                    "missing alternative bedGraph decompression provider "
                    "requirement"
                ),
...
def _runtime_entry_findings(
    ...
    """
    Validate one documented runtime-requirement entry.
    """

    findings: list[Finding] = []
    callable_message = exact_callable_message(value, callables, concepts)
    ...
    findings.extend(
        ...
        for rule_id, message in runtime_policy_messages(value, callables)
    )
    ...
    if parsed and parsed.group("name") not in {"bash", "python", "python3"}:
        findings.append(
            Finding(
                ...
                entry_line,
                (
                    "non-Bash/Python version claims need source-grounded "
                    "validation"
                ),
            ),
        )
    ...
    if (
        parsed
        and parsed.group("name") == "bash"
        and requirement_head != "bash >= 4.4"
    ):
        findings.append(
            Finding(
                RULE_VERSION,
                ...
                path,
                entry_line,
                "Bash must use the canonical minimum 'bash >= 4.4'",
            ),
        )
    ...
    if "env_protocol" in value:
        findings.append(
            Finding(
                ...
                entry_line,
                (
                    "env_protocol is an --env_nam default, not a Runtime "
                    "requirement"
                ),
            ),
        )
    ...
    if INTERNAL.search(value):
        findings.append(
            Finding(
                ...
                entry_line,
                (
                    "repository-local helpers, scripts, and modules are not "
    ...
    if " when " in value and condition is None:
        findings.append(
            Finding(
                ...
                entry_line,
                (
                    "conditions must use '<requirement> (when "
                    "<source-grounded trigger>)'"
                ),
            ),
        )
...
def _documented_runtime_findings(
    identity: str,
    path: str,
    line: int,
    entries: list[str],
    entries_with_lines: list[tuple[int, str]],
    evidence: list[Evidence],
    expected_requirements: tuple[str, ...],
    callables: set[str],
    concepts: dict[str, set[str]],
    ...
    """
    Validate a present Runtime requirements section in semantic phases.
    """

    normalized, findings = _runtime_shape_findings(
        identity,
        path,
        line,
        entries,
            ...
            if value not in expected_requirements:
                findings.append(
                    Finding(
                        ...
                        entry_line,
                        (
                            "entry is not part of the source-reviewed owner "
                            "contract"
                        ),
                        expected_requirements,
                        tuple(normalized),
...
def analyze_owner(
    ...
    ----------
    identity : str
        Stable owner identity used to associate evidence.
    unit : tuple[str, Path, Path, Heredoc]
        Discovered owner kind, source, documentation, and help heredoc.
    root : Path
        Repository root used to resolve maintained paths.
    callables : set[str] | None
        Callable identifiers accepted by runtime validation.
    concepts : dict[str, set[str]] | None
        Recognized runtime concepts and their accepted spellings.

    Returns
    -------
    inventory, findings, candidates : tuple[
        dict[str, object], list[Finding], list[dict[str, object]]
    ]
        Runtime-requirement inventory and findings for the owner.
    """
    ...
    if grammar:
        findings.extend(
            Finding(
                RULE_OWNER_SECTION
                if message == RUNTIME_PARENT_MESSAGE
                else RULE_GRAMMAR,
                identity,
                path,
                line,
    ...
    if label is None:
        findings.append(
            Finding(
                ...
                heredoc.start_line,
                (
                    "current owners inherit bash >= 4.4 and require a Runtime "
    ...
    candidates = [
        candidate(identity, item, entries)
    ...
    inventory = {
        ...
        "help_start_line": heredoc.start_line,
        "runtime_label_line": label,
        "documented_entries": entries,
...
def audit(
    root: Path,
) -> tuple[list[dict[str, object]], list[Finding], list[dict[str, object]]]:
    """
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """
    Parse Runtime-requirements audit arguments.
...
def main(argv: list[str] | None = None) -> int:
    """
    Audit and optionally normalize Runtime-requirements documentation.
    ...
    -------
    status : int
        Zero when strict findings are absent, otherwise one.
    """
    ...
    print(
        (
            f"Runtime requirements: {len(inventory)} owner(s), "
            f"{len(findings)} "
            f"finding(s), {len(candidates)} semantic "
            f"candidate(s)"
        ),
    )
