...
RULE_RUNTIME_PARENT = "HELP.RUNTIME.PARENT"
...
SHORT_OPTION = re.compile(
    r"(?<![-A-Za-z0-9_])-(?!-|\d)([A-Za-z0-9][A-Za-z0-9]*)",
)
...
GROUPED_GLOBAL = re.compile(
    r"^(?P<indent> *)(?P<head>[A-Za-z_][A-Za-z0-9_]*(?:, "
    r"[A-Za-z_][A-Za-z0-9_]*)+)\s+:\s+(?P<type>\S.*)$",
)
...
UNKNOWN_MESSAGE = re.compile(
    ...
    r"unknown (?:argument|option|option/parameter) passed)",
    re.I,
)
APPROVED_GLOBAL_TYPE = re.compile(
    ...
    r"structured string|"
    r"(?:array|list) of "
    r"(?:str|int|num|float|bool|file|dir|path|structured string)|\{[^{}]+\})$"
    r"",
)
CATEGORY_NAMES = {
    "Programs",
...
def case_arm_spacing_warnings(
    ...
    ----------
    text : str
        Shell source to inspect.
    path : str
        Repository-relative path reported in diagnostics.

    Returns
    -------
    advisories : list[Advisory]
        Stable advisories for adjacent recognized case arms.
    """
    ...
    for index, line in enumerate(lines[:-1]):
        ...
        warnings.append(
            Advisory(
                ...
                line=index + 2,
                message=(
                    "prefer exactly one empty line between adjacent case arms"
                ),
            ),
        )
...
def active_top_level_sections(heredoc: Heredoc) -> dict[int, str | None]:
    """
    Map each help-body line to its active recognized top-level section.
    """
...
def check_canonical_tables(
    ...
    """
    Check recognized Markdown-style tables in documentation content.
    """

    index = 0

    while index + 1 < len(lines):
        if split_pipe_row(lines[index][1]) is None:
            index += 1
        ...
        while end < len(lines) and split_pipe_row(lines[end][1]) is not None:
            end += 1

        table_lines = [line for _, line in lines[index:end]]
        ...
        if actual != format_table(table):
            numbers = tuple(number for number, _ in lines[index:end])
            add_finding(
                findings,
                RULE_TABLE,
                path,
                numbers[0],
                ("documentation table is not in canonical Markdown pipe form"),
                active_lines,
                *numbers[1:],
            )
    ...
    for group in groups[:1]:
        ...
        if (
            ...
            add_finding(
                ...
                first_number,
                (
                    "command or script name must appear alone at section base "
                    "+ 2 spaces"
                ),
                active_lines,
            )
        ...
        for number, line in group[1:]:
            if indentation(line) != first_indent + 2:
                add_finding(
                    ...
                    number,
                    (
                        "argument continuation must be exactly two spaces "
                        "deeper than the command name"
                    ),
                    active_lines,
                    first_number,
                )
        ...
        for number, line in group:
            ...
            if SHORT_OPTION.search(line):
                add_finding(
                    ...
                    number,
                    (
                        "public Usage must advertise canonical long option "
                        "names only"
                    ),
                    active_lines,
                )
...
def check_entries(
    ...
    ----------
    section : Section
        Parsed help section whose entries are checked.
    path : str
        Repository-relative path associated with the source.
    findings : list[Finding]
        Mutable finding collection populated by the check.
    active_lines : set[int] | None
        Source line numbers active after bounded conditional evaluation.
    """
    ...
    global_section = section.name in {
        "Expected globals",
        "Generated globals",
    }
    ...
    for index, (number, line) in enumerate(body):
        ...
        if match is None and global_section:
        ...
        if (
            ...
            add_finding(
                ...
                number,
                (
                    "grouped globals require valid identifiers separated by "
                    "comma and one space before one shared type"
                ),
                active_lines,
            )
        ...
        if match is None:
            if (
                ...
                add_finding(
                    ...
                    number,
                    (
                        f"{section.name} entries must be flat; remove "
                        f"'{line.strip()}'"
                    ),
                    active_lines,
                )
        ...
        if missing_type:
        ...
        elif global_section and (
            match.group("type").count(" : ")
            or re.search(
            ...
            add_finding(
                ...
                number,
                (
                    "grouped globals require one shared type after the "
                    "complete identifier list"
                ),
                active_lines,
            )
        elif global_section and not APPROVED_GLOBAL_TYPE.fullmatch(
            ...
            add_finding(
                ...
                number,
                (
                    f"{section.name} entry uses an unapproved type: "
            ...
            add_finding(
                ...
                number,
                (
                    "entry description must begin on the next line at row "
    ...
    for index, number, line, missing_type in rows:
        ...
        for _, description in body[index + 1 :]:
            ...
            if entry_match(section.name, description) or GROUPED_GLOBAL.match(
                description,
            ):
                break
        ...
        if GROUPED_GLOBAL.match(line):
            ...
            if not any(marker in lowered for marker in common_markers):
                add_finding(
                    ...
                    number,
                    (
                        "grouped-global description needs 'respectively' or "
                        "equally explicit shared wording"
                    ),
                    active_lines,
                )
    ...
    for (
        type_name,
        description,
    ), line_numbers in globals_by_description.items():
        if description and len(line_numbers) > 1:
            add_finding(
                ...
                line_numbers[1],
                (
                    f"separate {section.name.lower()} entries repeat one "
                    f"'{type_name}' "
                    f"description and should be grouped"
                ),
                active_lines,
                line_numbers[0],
            )
...
def exact_callable_message(
        ...
        if re.match(rf"^{re.escape(concept)}(?:\b|\s|[,;(])", text, re.I):
            ...
            if len(choices) == 1:
                ...
                if not text.startswith(callable_name):
                    return (
                        f"use exact callable spelling '{callable_name}', not "
                        f"'{concept}'"
                    )
    ...
    if token:
        ...
        for callable_name in callables:
            if (
                observed.lower() == callable_name.lower()
                and observed != callable_name
            ):
                return (
                    f"use exact callable spelling '{callable_name}', not "
                    f"'{observed}'"
...
def check_runtime_blocks(
    ...
    ----------
    heredoc : Heredoc
        Parsed help heredoc whose rendered contract is checked.
    path : str
        Repository-relative path associated with the source.
    findings : list[Finding]
        Mutable finding collection populated by the check.
    ...
    for label_index, (label_number, label_line) in enumerate(body):
        ...
        for index in range(label_index + 1, len(body)):
            ...
            if item is None:
                ...
                if (
                    old_name in OLD_CATEGORY_NAMES
                    and line_indent > label_indent
                ):
                    add_finding(
                        findings,
                        ...
                        number,
                        (
                            "runtime categories must use '- Category' with "
                            "'+' nested items"
                        ),
                        active_lines,
                        label_number,
            ...
            if marker == "-":
                ...
                if item_indent != label_indent + 2:
                    add_finding(
                        ...
                        number,
                        (
                            "direct runtime bullet must be exactly two spaces "
                            "deeper than its label"
                        ),
                        active_lines,
                        label_number,
                    )
                ...
                if not current_category or item_indent != label_indent + 4:
                    add_finding(
                        ...
                        number,
                        (
                            "nested runtime item must be exactly two spaces "
                            "deeper than its category"
                        ),
                        active_lines,
                        label_number,
                    )
        ...
        if len(direct) == 1 and direct[0][1] == "Programs" and direct[0][2]:
            add_finding(
                ...
                direct[0][0],
                (
                    "use a flat runtime list when Programs is the only "
            ...
            for number, _, is_category, bulleted in direct:
                if not bulleted and not is_category:
                    add_finding(
                        ...
                        number,
                        (
                            "two or more peer runtime requirements must use "
                            "bullets"
                        ),
                        active_lines,
                        label_number,
...
def is_function_help(heredoc: Heredoc) -> bool:
    ...
    return heredoc.owner not in {
        "<file>",
        "<rendered>",
        "main",
    } and not heredoc.owner.startswith(("help_", "show_help", "detail_"))
...
def check_document_structure(
    ...
    for number, line in body:
        if (
            RUNTIME_LABEL.fullmatch(line)
            and active_sections.get(number) != "Notes"
        ):
            add_finding(
                findings,
                RULE_RUNTIME_PARENT,
                path,
                number,
                "'Runtime requirements:' must occur inside the active "
                "top-level Notes section",
                active_lines,
            )
        ...
        for number, line in section.lines:
            if line.strip() and indentation(line) < section.heading_indent + 2:
                add_finding(
                    ...
                    number,
                    (
                        "top-level section content must begin at section base "
                        "+ 2 spaces"
                    ),
                    active_lines,
                    section.heading_line,
    ...
    for name, lines in names.items():
        for duplicate in lines[1:]:
            add_finding(
                ...
                duplicate,
                (
                    f"top-level heading '{name}' appears more than once in "
                    f"one help unit"
                ),
                active_lines,
                lines[0],
    ...
    for index, (number, line) in enumerate(body):
        ...
        if stripped.endswith(":") or underlined:
            add_finding(
                ...
                number,
                (
                    "replace Dependency/Dependencies with Notes and 'Runtime "
                    "requirements:'"
                ),
                active_lines,
            )
            ...
            for content_number, content_line in lines[index + 1 :]:
                ...
                if indentation(content_line) <= indentation(line):
                    add_finding(
                        ...
                        content_number,
                        (
                            "content beneath a top-level Notes pseudo-header "
                            "must be exactly two spaces deeper"
                        ),
                        active_lines,
                        number,
                    )
    ...
    if (
        ...
        add_finding(
            ...
            usage.heading_line,
            (
                "function help documenting arguments requires a 'Parameters' "
                "section"
            ),
            active_lines,
    ...
    ordered = [
        section for section in parsed if section.name in FUNCTION_SECTION_RANK
    ]
    ...
    for later_index, later in enumerate(ordered):
        ...
        if conflicts:
            ...
            add_finding(
                ...
                later.heading_line,
                (
                    f"'{later.name}' must precede '{earlier.name}' in "
                    f"function help"
                ),
                active_lines,
    ...
    if examples is not None and parsed[-1].name != "Examples":
        add_finding(
            ...
            (
                f"'{parsed[-1].name}' may not appear after 'Examples'; "
                f"Examples must be final"
            ),
            active_lines,
        ...
        for number, line in section.lines:
            ...
            if match:
                ...
                add_finding(
                    ...
                    number,
                    (
                        "use straight single quotes as prose delimiters in "
                        "shell help"
                    ),
                    active_lines,
                )
...
def check_parser_messages(
    ...
    for number, line in enumerate(text.splitlines(), 1):
        if (
            ...
            r"TODO.*(?:Unknown argument|unknown option|echo_err)",
            line,
            re.I,
        ):
            add_finding(
                ...
                number,
                (
                    "remove the stale TODO for the settled unknown-option "
                    "diagnostic"
                ),
                active_lines,
            )
...
def check_python_docstrings(
    ...
    active_lines : set[int] | None
        Optional changed-line restriction.

    Returns
    -------
    findings : list[Finding]
        Canonical-table and prose-delimiter findings outside examples.
    """
    ...
    for node in ast.walk(tree):
        ...
        decoded_lines = tuple(
            (start + index, line)
            for index, line in enumerate(expression.value.value.splitlines())
        )

        check_canonical_tables(
            decoded_lines,
            path,
            findings,
            active_lines,
...
def check_rendered_help(
    ...
    *,
    ...
    check_canonical_tables(heredoc.lines, path, findings, None)
...
def scan_repository(
    ...
    *,
    ...
    if paths is not None:
        ...
        selected = (
            shell_paths(root) + tracked_python_paths + untracked_python_paths
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    arguments : argparse.Namespace
        Parsed help-style audit options.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    -------
    status : int
        Zero when help structure passes and one when findings remain.
    """
