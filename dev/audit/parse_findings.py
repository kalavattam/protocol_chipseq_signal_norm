...
def parse_pilot_payload(
    ...
    ----------
    result : CommandResult
        Completed adapter process, including stdout and launch state.

    Returns
    -------
    payload : dict[str, list[dict[str, object]]]
        Normalized facts, findings, policy questions, and limitations.

    Raises
    ------
    ValueError
        If successful adapter output is not the versioned object contract.
    """
    ...
    try:
        ...
        raise ValueError(
            f"pilot adapter emitted invalid JSON: {exc.msg}",
        ) from exc

    if not isinstance(payload, dict) or payload.get("schema_version") != 1:
        raise ValueError(
            "pilot adapter payload must be a schema_version 1 object",
        )
    ...
    for key in ("facts", "findings", "policy_questions", "limitations"):
        ...
        if not isinstance(value, list) or not all(
            ...
            raise ValueError(
                f"pilot adapter payload {key} must be a list of objects",
            )
