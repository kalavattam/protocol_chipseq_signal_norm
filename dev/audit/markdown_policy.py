...
def check_standard_sections(text: str) -> list[Finding]:
    ...
    for start in rule_sections:
        ...
        for required in STANDARD_FIELDS:
            positions = [
                index for label, index in fields if label == required
            ]
            if not positions:
                findings.append(
                    Finding(
        ...
        adjacent = (
            ...
            and (field := STANDARD_FIELD.match(lines[start + 1]))
            is not None
...
def _anchor_findings(
    ...
    for index in candidates:
            ...
            if not identifier or any(
                ...
                findings.append(
                    Finding(
                        "MD.ANCHOR.CANONICAL",
                        index + 1,
                        "decoded anchor ID must be nonempty and contain no ASCII whitespace",
                    )
                )
        ...
        if (
            following < len(lines)
            and ANCHOR_CANDIDATE.fullmatch(lines[following])
        ):
            findings.append(
                Finding(
                    "MD.ANCHOR.CANONICAL",
...
def _section_findings(
    ...
    for position, unit in enumerate(units[1:], 1):
        ...
        if direct_child:
            ...
            valid = actual == []
            message = (
                "contentless direct parent and child must have no separator or blank"
            )
        elif (
            last_real is not None
            and lines[last_real].lower() == "</details>"
            and _boundary_tokens(
                lines,
                last_real + 1,
                segment_end,
                literal,
            ...
            valid = actual == ["<br />", ""]
            message = (
                "details-close boundary must use its direct break with no second separator"
            )
        else:
            boundary_start = (
                last_real + 1
        ...
        if following < len(lines) and blanks != expected:
            findings.append(
                Finding(
                    "MD.HEADING.SPACING",
                    unit.title + 1,
                    "heading needs one blank before a table or ordinary section break and none otherwise",
                )
            )
    return findings
...
def check_explicit_links(
    ...
    resolved_texts = {file_path.resolve(): text for file_path, text in texts.items()}
    identifiers = {
        ...
        if first_nonempty != units[0].start:
            findings.append(
                Finding(
                    "MD.FILE.BOUNDARY",
                    1,
                    "first heading unit must immediately follow the leading line feed",
                )
            )
    ...
    while index < len(lines):
        ...
        if line.rstrip().endswith(":"):
            ...
            if following < len(lines):
                ...
                table_follows = (
                    following + 1 < len(lines)
                    and parse_table(lines[following : following + 2])
                    is not None
                )
        ...
        if LIST.match(line):
            indent = LIST.match(line).group("indent")  # type: ignore[union-attr]
            if len(indent) % 2:
                findings.append(
                    Finding(
                        "MD.LIST.INDENT",
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 1 if any(
        item["classification"] == "deterministic" for item in output
    ) else 0
