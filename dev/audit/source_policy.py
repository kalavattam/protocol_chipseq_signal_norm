...
from help_heredoc_reflow import shell_paths

SOURCE_LINE = re.compile(r"^\s*(?:source|\.)\s+(?P<expression>.+?)\s*(?:\|\||$)")
DISABLE = re.compile(r"^\s*#\s*shellcheck\s+disable=(?P<codes>SC\d+(?:,SC\d+)*)\s*$")
...
SOURCE_CALL = re.compile(r"^\s*(?:source|\.)\s+[^#\n]*submit_[A-Za-z0-9_]+\.sh")
COMPAT_EXEC = re.compile(
    r'^exec "\$\{BASH\}" "\$\{dir_scr\}/(?P<target>submit_[A-Za-z0-9_]+\.sh)" "\$@"$'
)
...
def _maintained_reference_paths(root: Path) -> list[Path]:
    ...
    return sorted({path for path in paths if "artifacts/tests" not in path.parts})
...
def _reference_inventory(
    ...
    for path in _maintained_reference_paths(root):
        ...
        if any(SOURCE_CALL.search(line) for line in matching):
            ...
            relative.startswith("bin/execute_")
            and any("scr_sub=" in line for line in matching)
        ):
            direct.add(relative)
...
def discover_submit_interfaces(root: Path) -> list[SubmitInterface]:
    ...
    for path in sorted((root / "bin").glob(SUBMIT_GLOB)):
        ...
        exec_match = next(
            (COMPAT_EXEC.match(line.strip()) for line in lines if COMPAT_EXEC.match(line.strip())),
            None,
        )
        guard = next(
            (
        ...
        guard_before = (
            exec_match is not None
            or (guard is not None and first_code is not None and first_code <= guard)
        )
        execute_owner = root / "bin" / path.name.replace("submit_", "execute_", 1)
        ...
        if exec_match is not None:
            ...
            disposition = (
                "retain as a non-executable compatibility delegator with the canonical "
                "Bash shebang and exact argument/status-preserving exec"
            )
        else:
            classification = "worker-only interface"
            modes: list[str] = []
            owner_text = execute_owner.read_text(encoding="utf-8") if execute_owner.is_file() else text
            if "sbatch" in owner_text or "SLURM_ARRAY_TASK_ID" in text:
                modes.append("Slurm")
            if "parallel" in owner_text:
                modes.append("GNU Parallel")
            if '"${BASH}"' in owner_text or "run_local" in text or "run_job" in text:
                modes.append("local")
            behavior = "/".join(modes) + " worker dispatch"
            helper = "bootstrap help source, then source_helpers_submit/source_helpers"
            environment = (
            ...
            disposition = (
                "retain as a non-executable worker entry point with the canonical Bash "
                "shebang, an immediate Bash >= 4.4 guard, and explicit-interpreter/Slurm dispatch"
            )
...
def submit_bootstrap_findings(root: Path) -> tuple[list[Advisory], list[SubmitInterface]]:
        ...
        if row.executable_bit:
            findings.append(
                Advisory(
                    row.path,
                    1,
                    "submit interfaces are interpreter/Slurm entry points and must remain non-executable",
                    "SHELL.SUBMIT.MODE",
                )
            )
            ...
            if exec_match is None or exec_match.group("target") not in by_name:
                findings.append(
                    Advisory(
                        row.path,
                        len(lines),
                        "compatibility wrapper must exec a maintained submit target via '${BASH}' and exact '$@'",
                        "SHELL.SUBMIT.COMPATIBILITY",
                    )
                )
            forbidden = re.compile(r"\b(?:source|conda|handle_env|setup_env)\b")
                ...
                if forbidden.search(line) and not line.lstrip().startswith("#"):
                    findings.append(
                        Advisory(
                            row.path,
                            index,
                            "compatibility wrapper must delegate before helper or environment bootstrap",
                            "SHELL.SUBMIT.COMPATIBILITY",
                        )
                    )
            continue

        guard_index = row.bash_guard_location - 1 if row.bash_guard_location else None
        if guard_index is None or first_code is None or not lines[first_code].startswith("if [["):
            findings.append(
                Advisory(
                    row.path,
                    1,
                    "worker must begin with the Bash >= 4.4 guard before other source constructs",
                    "SHELL.SUBMIT.GUARD_ORDER",
                )
            )
            continue
        end = _guard_end(lines, first_code)
        if end is None:
            findings.append(
                Advisory(
                    row.path,
            ...
            if fragment.lower() not in guard_text.lower():
                findings.append(
                    Advisory(
                        row.path,
                        first_code + 1,
                        f"Bash guard missing canonical boundary/diagnostic fragment: {fragment}",
                        "SHELL.SUBMIT.GUARD_CONTRACT",
                    )
                )
        pre_guard = "\n".join(lines[first_code:guard_index])
        if re.search(r"\b(?:source|conda|handle_env|setup_env|local -n|declare -gA)\b", pre_guard):
            findings.append(
                Advisory(
                    row.path,
                    first_code + 1,
                    "helper/environment bootstrap or Bash >= 4.4 syntax precedes the version boundary",
                    "SHELL.SUBMIT.GUARD_ORDER",
                )
            )
        if not re.search(r'^main "\$@"\s*$', text, re.MULTILINE):
            findings.append(
                Advisory(
                    row.path,
                    len(lines),
...
def classify_source(path: str, line: str) -> SourceRecord:
    ...
    if value.startswith('"$(') and path in {
        "lib/bash/workflows/filter_bam.sh",
        "lib/bash/workflows/filter_cram.sh",
    }:
        return SourceRecord("static", "lib/bash/workflows/filter_alignment.sh", None)
    if value.startswith('"$(') and path == "lib/bash/help/help_execute_filter_bams.sh":
        return SourceRecord(
            "static",
...
def preceding_source_comments(lines: list[str], index: int) -> tuple[set[str], str | None]:
            ...
            if directive != record.target:
                findings.append(
                    Advisory(
                        ...
                        f"'shellcheck source={record.target}' before the statement",
                    )
                )
            stale = sorted(code for code in disables if code in {"SC1090", "SC1091"})
            if stale:
                findings.append(
                    Advisory(
                        path,
                        index + 1,
                        f"static source has stale {','.join(stale)} suppression; use source= mapping",
                    )
                )
        elif record.required_suppression and record.required_suppression not in disables:
            findings.append(
                Advisory(
                    path,
                    index + 1,
...
def bootstrap_help_spacing_warnings(path: str, text: str) -> list[Advisory]:
    ...
    for index, line in enumerate(lines):
        if "source " not in line or "help_submit_" not in line:
            continue
        cursor = index - 1
        while cursor >= 0 and (DISABLE.match(lines[cursor]) or DIRECTIVE.match(lines[cursor])):
            cursor -= 1
        blanks = 0
        ...
        if blanks != 1:
            findings.append(
                Advisory(
                    path,
                    index + 1,
                    "prefer exactly one blank line between the completed bootstrap "
                    "help-path assignment and its ShellCheck/source statement",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.help_source_spacing:
        print(f"SHELL.HELP_SOURCE.SPACING: {len(warnings)} advisory warning(s)")
        return 0
    if warnings:
