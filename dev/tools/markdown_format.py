...
def maintained_markdown(root: Path) -> list[Path]:
    ...
    # Markdown fixtures need no exclusion here. They are generated by
    # 'tests/fixtures/markdown/make.sh' and ignored, and this discovery lists
    # tracked and nonignored untracked files, so a deliberately rejected
    # document is already outside the maintained corpus. An exclusion tuple
    # would be unreachable code that reads like a live boundary.
    return sorted(
        file_path
