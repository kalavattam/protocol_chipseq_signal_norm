...
**Automation:** `dev/audit/shell_source_form.py` reports the retired `#  ` and `#+ ` ordinary-comment markers outside the bounded source header, ShellCheck directives, and heredoc bodies. That facet exists because the marker is the one part of this rule a checker can decide alone, and because new source kept acquiring the retired form by imitating unmigrated neighbours rather than by reading this rule. Its repository-wide count is the `S3-MIG-005` migration inventory, not a blocking gate. No checker implements the remainder: attachment, prose quality, and the distinction between an ordinary comment and separately owned syntax stay review-owned, and no migration fixture set exists. Both inline-alignment bounds are measurable and could be checked once a facet exists; none reports them today, so an aligned run is review-owned like the rest of this rule.
