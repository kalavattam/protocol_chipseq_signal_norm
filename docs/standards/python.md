...
A maintained CLI module docstring presents its interface as a synopsis example whose operand order follows [`HELP.EXAMPLES`](help.md). Python realizes that synopsis with a module-invocation or installed-command first line, one escaped line continuation, and a continuation row indented four spaces; the generic `[options]` placeholder ends the operand row. Inside a docstring the continuation is written `\\` so the rendered value carries one backslash.
...
Docstring prose paragraphs wrap greedily under [`SOURCE.PROSE.WRAP`](source_layout.md), which is the sole diagnostic owner for a premature docstring break. This rule owns the structural boundaries that are never prose: canonical section headers and their underlines, `name : type` entry headers, multiline textual-type rows, `Examples` sections, and doctest rows.
...
The reviewed Python vocabulary permits unambiguous domain and ecosystem terms such as `BAM`, `CRAM`, `IP`, `SIQ`, `bedGraph`, `CSV`, `TSV`, `JSON`, `gzip`, `CLI`, `SSH`, `cwd`, and the conventional imported NumPy alias `np`. Existing names that mirror protected CLI destinations may retain project spellings such as `fil_in`, `fil_out`, `siz_bin`, `skp_pfx`, `scl_fct`, `frg`, `psc`, and `qntl` only at that interface or in code directly adapting it. New implementation locals use descriptive words. In particular, do not introduce private shorthand segments such as `fh`, `py`, `sb`, `str`, `xs`, or `ys`; use names such as `handle`, `python`, `start_bins`, `text`, or `values`, or another truthful domain name.
