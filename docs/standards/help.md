...
A command with `--details` owns its required full document there, so its concise `--help` Examples ordinarily demonstrate the help function itself. When that concise surface is registered in `dev/config/help_contracts.json` with `example_form` `rendered_cli_invocation` and the wrapper script as `owner`, its block instead carries rendered CLI invocations of the wrapper, held to the same structure, distinctness, and exact-preservation obligations as any other rendered CLI block. The registration records the approving authority; `owner_authorized_extension` records that the owner authorized extending the established examples pattern to that surface. An unregistered concise surface of a `--details` command keeps the ordinary owner-invocation obligation unchanged: registration opens a reviewed door, it does not waive the requirement.
...
| `report_n_ovlp` | file              | Overlap-count report path.                                                 |
