...
**`make.sh` generates. Tests and checkers validate. The separation binds in both directions, and a violation either way is a defect rather than a permitted variant.** A recipe authors its inputs literally and derives everything else from them; it never asserts what a checker should report about its output, because that assertion belongs to the test that consumes the fixture. Equally, a test never builds a fixture corpus of its own. A test that writes a set of input files into a scratch directory has written a recipe inside an assertion, where it is invisible to the fixture inventory, unavailable to any other test, and regenerated on every run rather than authored once and reviewed.

The boundary is the corpus, not the file write. A test may construct a bounded perturbation of a fixture it consumes — a single altered line, a mutated field, a case built to fail one rule — because the perturbation is the assertion and belongs beside it. What belongs in a recipe is a set of inputs that stands on its own, that a second test could reuse, or that a person would want to read on disk.

**This direction is an open migration.** Tests that construct their inputs on disk predate the rule and are being converted; the source-derived inventory owns the current count. Until a test is converted, its inline construction is recorded work rather than a new permission, and no new test may add to it.

**A fixture root subdivides by the role each file plays in the assertion, never by where the file came from.** Where a checker returns a verdict, the roles are the verdicts, and the directory names one of `accepted/`, `rejected/`, `boundary/`, or `non_applicable/`. That vocabulary is already the repository's: every fixture `cases.json` keys on exactly those four readings, so expressing them as directories only makes a taxonomy the tests already use visible in the tree. A verdict with no fixture is then an absent directory rather than a filename nobody happened to write.

Where a file plays some other role, the directory names that role instead: `format/` for the input and expected pairs of a rewrite, `expected/` for a golden output, `tool/` for an executable stub, `script/` for an input whose language is the subject. Workflow inputs name the kind of data they hold — `reference/`, `fastq/`, `bam/`, `sam/`, `bedgraph/` — because that is the role a workflow input plays. Do not name a directory `source/`: it records provenance rather than role, so it accepts anything and constrains nothing.

**A golden file is authored, never derived.** "Author text and derive everything else" stops at expected output. Regenerating a golden with the same producer the test runs leaves the test comparing a producer against itself, which passes for every output including an empty one.
...
**Automation:** `tests/contract/repository/test_fixture_paradigm.sh` asserts every deterministic obligation above for each fixture root: ignore stanzas, tracked-file boundaries, runner wiring, recipe shape, the generate-not-validate separation, an explicit stale sweep, README sections, and regeneration idempotence. Idempotence is proved rather than argued: every recipe runs twice and its outputs are compared by digest, including the recipes that need the managed environment. A recipe that stops on a missing prerequisite is a counted skip naming the prerequisite, never a silent pass. The contract derives the root population from the filesystem and from Git independently and asserts the inspected count, so a root it cannot see is a failure rather than a silent omission. Roots with no recipe are reported as a counted warning. `test_test_layout.sh` retains production independence, which is a claim about a different corpus.
