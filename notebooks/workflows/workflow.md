...
*Note: If using a high-performance computing (HPC) system, it’s recommended to request an interactive node to ensure adequate resources for running code in the below chunks. How you do this depends on how your job scheduler&mdash;the software for managing work on an HPC&mdash;is set up. (At Fred Hutch Cancer Center, run `grabnode` and complete the interactive prompts for job parameters.) Note this step is unnecessary if running the code on a local machine.*
...
1. [Data analysis](#data-analysis)
    ...
    1. [F. Compute normalized coverage.](#f-compute-normalized-coverage)
        1. [Text](#text-3)
        1. [Bash code](#bash-code-compute-normalized-coverage)
    1. [G. Construct sample tables recording computed scaling factors for normalization.](#g-construct-sample-tables-recording-computed-scaling-factors-for-normalization)
        1. [1. Construct sample table recording siQ-ChIP $\alpha$ scaling factors.](#1-construct-sample-table-recording-siq-chip-alpha-scaling-factors)
            1. [Text](#text-4)
            1. [Bash code](#bash-code-construct-siq-chip-alpha-scaling-table)
...
[Miniforge](https://github.com/conda-forge/miniforge) is an open source tool for managing bioinformatics software in environments, self-contained workspaces that prevent software conflicts. This protocol uses Miniforge to create and manage an environment, `env_protocol`, that contains programs for read alignment, alignment processing, signal computation, signal visualization, and more. It is recommended to [uninstall other software managers (e.g., Anaconda)](https://stackoverflow.com/questions/42182706/how-to-uninstall-anaconda-completely-from-macos) before installing Miniforge.
</details>
<br />
...
#  Define code-chunk name
chunk="Procedure A: Install and configure Miniforge"
...
cd "${dir_bas}" || {
    echo "error(${chunk}): failed to change directory: '${dir_bas}'." >&2
    exit 1
}
...
Use the script [`install_envs.sh`](../../install/scripts/install_envs.sh) to set up the project environment, `env_protocol`.
</details>
<br />

<details>
<summary><i>Bash code: Clone the protocol repository and install the project environment</i></summary>
...
chunk=(
    "Procedure B: Clone the protocol repository and install the project"
    "environment"
)
...
cd "${repo}" || {
    echo "error(${chunk[*]}): failed to change directory: '${repo}'." >&2
    exit 1
}

#  Install the Conda/Mamba environment and editable Python package
bash "install/scripts/install_envs.sh" --env_nam "${env_nam}" --yes
```
</details>
<br />
...
#  Load the maintained shared Bash library
...
bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_idx}/logs"
...
if "${slurm:-false}"; then
    check_pgrm_path sbatch &> /dev/null ||
        echo_warn \
...
#+ logs
mkdir -p "${dir_raw}" "${dir_sym}" "${dir_log}"
...
if ! \
    "${cmd_download[@]}" \
         > >(tee -a "${dir_log}/${day}.execute.stdout.txt") \
        2> >(tee -a "${dir_log}/${day}.execute.stderr.txt")
then
    echo "error(workflow): download_fastqs failed; cleanup was not run." >&2
    exit 1
fi
...
#  Cleanup: Compress logs and remove empty files ------------------------------
if ! \
    bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_log}"
then
    echo "error(workflow): log cleanup failed." >&2
    exit 1
fi
# ls -lhaFG "${dir_log}"  ## Uncomment to check log directory ##
```
</details>
<br />
...
if [[ "${debug:-false}" == "true" ]]; then
    ...
    echo "###########################"
    echo ""
    echo "bash ${dir_bin}/execute_trim_fastqs.sh \\"
    echo "    --verbose \\"
    echo "    --threads ${threads} \\"
    echo "    --csv_fil_in ${infiles} \\"
    echo "    --dir_out ${dir_trm} \\"
    echo "    --dir_eo ${dir_trm}/logs \\"
    echo "    --max_job ${max_job} \\"
...
bash "${dir_bin}/execute_trim_fastqs.sh" \
    --verbose \
    --threads "${threads}" \
    --csv_fil_in "${infiles}" \
    --dir_out "${dir_trm}" \
...
if [[ "${debug:-false}" == "true" ]]; then
    ...
    echo "###########################"
    echo ""
    echo "bash ${dir_bin}/execute_align_fastqs.sh \\"
    echo "    --verbose \\"
    echo "    --threads ${threads} \\"
    echo "    --aligner ${aligner} \\"
    echo "    --bt2_mode ${a_type} \\"
    echo "    --mapq ${mapq} \\"
    echo "    $(if ${req_flg:-false}; then echo --req_flg; fi) \\"
    echo "    --index ${stm_idx} \\"
    echo "    --csv_fil_in ${infiles} \\"
    echo "    --dir_out ${dir_out}/init \\"
    echo "    --dir_eo ${dir_out}/init/logs \\"
    echo "    --max_job ${max_job} \\"
    echo "    $(if ${slurm:-false}; then echo --slurm; fi) \\"
...
bash "${dir_bin}/execute_align_fastqs.sh" \
    --verbose \
    --threads "${threads}" \
    --aligner "${aligner}" \
    --bt2_mode "${a_type}" \
...
if [[ "${debug:-false}" == "true" ]]; then
    ...
    echo "###########################"
    echo ""
    echo "bash ${dir_bin}/execute_filter_alignments.sh \\"
    echo "    --verbose \\"
    echo "    --threads ${threads} \\"
    echo "    --csv_fil_in ${infiles} \\"
    echo "    --dir_out ${dir_out}/sc \\"
    echo "    --retain sc \\"
    echo "    --dir_eo ${dir_out}/sc/logs \\"
    echo "    --max_job ${max_job} \\"
    echo "    $(if ${slurm:-false}; then echo --slurm; fi) \\"
...
bash "${dir_bin}/execute_filter_alignments.sh" \
    --verbose \
    --threads "${threads}" \
    --csv_fil_in "${infiles}" \
    --dir_out "${dir_out}/sc" \
    --retain "sc" \
    --dir_eo "${dir_out}/sc/logs" \
    --max_job "${max_job}" \
    $(if ${slurm:-false}; then echo "--slurm"; fi) \
...
bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_out}/init/logs"
bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_out}/sc/logs"
bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_out}/sp/logs"
...
In the corresponding [Bash code chunk](#bash-code-compute-normalized-coverage), the signal type is specified by setting `typ_sig` to `"norm"` (normalized coverage). Other options are `"unadj"` (unadjusted binned coverage) and `"frag"` (fragment length-adjusted binned coverage; e.g., see [Dickson et al., *Sci Rep*, 2023](https://www.nature.com/articles/s41598-023-34430-2)). For details, see below and the [`compute_signal.py` documentation](https://github.com/kalavattam/protocol_chipseq_signal_norm/blob/main/src/protocol_chipseq_signal_norm/cli/compute_signal.py). BEDGRAPH and log output files are organized by signal type.
...
<a id="bash-code-compute-normalized-coverage"></a>
#### Bash code
<details>
...
#  Set execution parameters and related variables
env_nam="env_protocol"
scr_exc="${dir_bin}/execute_compute_signal.sh"
log_exc="${dir_log}/$(date '+%Y-%m%d').$(basename "${scr_exc}" ".sh")"

threads=6
csv_fil_in="$(  ## WARNING: Change search parameters as needed ##
    bash "${dir_bin}/find_files.sh" \
        --dir_fnd "${dir_bam}" \
        --pattern "*.bam"
)"
...
source_helpers \
    "${dir_bash}" \
    check_env check_unity format_outputs handle_env
...
#  Cleanup: Compress logs and remove empty files ------------------------------
bash "${dir_bin}/compress_remove_files.sh" --dir_fnd "${dir_eo}"
# ls -lhaFG "${dir_eo}"  ## Uncomment to check directory for logs ##
```
</details>
<br />
...
<a id="1-construct-sample-table-recording-siq-chip-alpha-scaling-factors"></a>
...
This section builds the siQ-ChIP alpha table consumed by later ratio workflows. The current `execute_calculate_scaling_factor.sh --mode siq` interface receives matched main-organism IP/input BAM lists plus the metadata table and parsing configuration, then writes the assembled TSV.
</details>
<br />

<a id="bash-code-construct-siq-chip-alpha-scaling-table"></a>
##### Bash code
<details>
...
typ_sig="siq"
env_nam="env_protocol"
scr_exc="${dir_bin}/execute_calculate_scaling_factor.sh"
log_exc="${dir_log}/$(date '+%Y-%m%d').$(basename "${scr_exc}" ".sh")"

threads=6
csv_mip="$(  ## WARNING: Change search parameters as needed ##
    bash "${dir_bin}/find_files.sh" \
        --dir_fnd "${dir_bam}" \
        --pattern "*.bam" \
        --include "IP*" \
...
csv_min="$(sed 's:\/IP:\/in:g' <<< "${csv_mip}")"
tbl_met="${dir_dat}/raw/docs/measurements_siqchip.tsv"
cfg_met="${dir_dat}/raw/docs/parse_metadata_siqchip.yml"
eqn="6nd"
...
This section builds the spike-in scaling table consumed by the downstream signal workflow. The current `execute_calculate_scaling_factor.sh --mode spike` interface receives matched main- and spike-organism IP/input BAM lists and uses its documented default spike-in method unless `--method` is supplied explicitly.
</details>
<br />
...
csv_sip="$(sed 's:sc:sp:g' <<< "${csv_mip}")"
csv_min="$(sed 's:\/IP:\/in:g' <<< "${csv_mip}")"
csv_sin="$(sed 's:\/IP:\/in:g' <<< "${csv_sip}")"
fil_out="${dir_tbl}/ChIP_WT_G1-G2M-Q_Hho1-Hmo1_${typ_sig}.tsv"
dir_eo="${dir_log}"
...
if [[ "${debug:-false}" == "true" ]]; then
    ...
    echo "slurm=${slurm}"
    echo "csv_mip=${csv_mip}"
    echo "csv_sip=${csv_sip}"
    echo "csv_min=${csv_min}"
    echo "csv_sin=${csv_sin}"
    echo "fil_out=${fil_out}"
    echo "dir_eo=${dir_eo}"
    echo ""
    echo ""
...
2.  *Activate environment and check dependencies:* Ensure required software (GNU Parallel, Python, etc.) is available.
3.  *Compute $\log_{2}$ ratios:* Call `execute_compute_signal.sh` with `--mode ratio --method log2` to generate $\log_2 \left( \text{IP} / \text{input} \right)$ BEDGRAPH tracks.
...
# shellcheck disable=SC1091
source "${dir_bash}/core/source_helpers.sh"
source_helpers "${dir_bash}" workflows/process_tables
...
check_pgrm_path awk
check_pgrm_path compute_input_floor
check_pgrm_path python
...
dir_eo="${dir_log}"
nam_job="compute_ratio_${typ_sig}"
...
if [[ "${debug:-false}" == "true" ]]; then
    echo "#####################################################"
    echo "## Call to driver script with '--csv_dep_min' argument ##"
    echo "#####################################################"
    echo ""
    echo "bash ${scr_exc} \\"
    echo "    --verbose \\"
...
#  Run driver script with '--csv_dep_min': Add '--dry_run' to view
...
#  Run driver script without '--csv_dep_min': Add '--dry_run' to
...
3. *Apply alpha scaling factors:* Read the siQ-ChIP alpha table from Section G and call `execute_compute_signal.sh` with `--mode ratio --method unadj`.
4. *Generate comparison tracks:* Produce ratio tracks both with and without the configured denominator floor.
...
##### Important note
- The upstream [`execute_calculate_scaling_factor.sh`](https://github.com/kalavattam/protocol_chipseq_signal_norm/blob/main/bin/execute_calculate_scaling_factor.sh) step used to create the alpha table requires that *S. cerevisiae* IP BAM files follow a specific naming convention as described in the protocol manuscript. The expected filename format:
    ```txt
...
dir_out="${dir_alf}"
csv_scl_fct="$(extract_field_str "${tbl_alf}" 3 true)"
csv_dep_min="$(
    compute_input_floor \
        --siz_bin "${siz_bin}" \
        --siz_gen "${siz_gen}" \
...
if [[ "${debug:-false}" == "true" ]]; then
    ...
    echo ""
    echo "typ_sig=${typ_sig}"
    echo "ratio_method=${ratio_method}"
...
This section describes the steps to calculate spike-in normalized ChIP-seq signal. For more details, see the introduction to [Section G](#g-construct-sample-tables-recording-computed-scaling-factors-for-normalization). The spike-in table supplies scaling factors to `execute_compute_signal.sh`, which runs in ratio mode with the unadjusted ratio method.
</details>
<br />
...
dir_out="${dir_spk}"
csv_scl_fct="$(extract_field_str "${tbl_spk}" 5 true)"
csv_dep_min="$(
    compute_input_floor \
        --siz_bin "${siz_bin}" \
        --siz_gen "${siz_gen}" \
