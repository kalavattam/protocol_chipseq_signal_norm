...
#  Set the path to the maintained entrypoint directory
...
----------------
  ...
  list_job_acc : array of str
    Parsed accession values.

  list_job_url_1 : array of str
    Parsed read-1 URL values.

  list_job_url_2 : array of str
    Parsed read-2 URL values.
  ...
  list_job_cus : array of str
    Parsed custom-name values.
...
-----------------
  cmd_bld : array of str
    Indexed array containing one call to 'submit_download_fastqs.sh'.
...
Examples
--------
  1. Display the builder contract before wrapper state is initialized.
    '''bash
    build_cmd --help
    '''

  2. Build the first prepared download job and inspect its command array.
    '''bash
    build_cmd 0
    declare -p cmd_bld
    '''
...
function validate_args() {
    ...
    dir_out="$(cd "${dir_out}" > /dev/null 2>&1 && pwd -P)" || {
        echo_err "failed to resolve output directory: '${dir_out}'."
        return 1
    }

    validate_var_dir "dir_sym" "${dir_sym}" || return 1
    dir_sym="$(cd "${dir_sym}" > /dev/null 2>&1 && pwd -P)" || {
        echo_err "failed to resolve symlink directory: '${dir_sym}'."
        return 1
    }
    ...
    dir_eo="$(cd "${dir_eo}" > /dev/null 2>&1 && pwd -P)" || {
    ...
    while IFS=$'\t' read -r line || [[ -n "${line}" ]]; do
        ...
        if [[ "${iter}" -eq 1 ]]; then
            ...
            if [[ -z "${run_acc_idx}" ]]; then
                echo_err \
                    "required column 'run_accession' was not found in header."
                return 1
            elif [[ -z "${custom_name_idx}" ]]; then
                echo_err \
                    "required column 'custom_name' was not found in header."
                return 1
...
function parse_jobs_uniq() {
    ...
    if [[ "${slurm}" == "true" && "${has_dup_dl}" == "true" ]]; then
        echo_err \
            "duplicate run_accession rows require local execution so" \
            "symlinks for all custom_name values can be created after the" \
            "unique download finishes."
        return 1
    fi
}
...
function prepare_config_parallel() {
    ...
    if [[ -f "${config}" ]]; then
        rm -- "${config}" || {
            echo_err "failed to reset GNU Parallel config: '${config}'."
            return 1
        }
    fi
    ...
    if [[ "${slurm}" == "true" ]]; then
        if [[ "${threads}" -gt 1 ]]; then
            ...
            if [[ "${dry_run}" == "false" ]]; then
                "${cmd_slurm[@]}" || {
                    echo_err "Slurm submission failed."
                    return 1
                }
            fi
        else
        ...
        if [[ "${threads}" -gt 1 ]]; then
            if [[ "${dry_run}" == "true" || "${verbose}" == "true" ]]; then
                ...
                parallel --jobs "${threads}" --dryrun < "${config}" || {
                    echo_err "GNU Parallel preview failed."
                    return 1
                }
                echo
                echo
            ...
            if [[ "${dry_run}" == "false" ]]; then
                parallel --jobs "${threads}" < "${config}" || {
                    echo_err "GNU Parallel execution failed."
                    return 1
                }
            ...
            for i in "${!list_job_acc[@]}"; do
                ...
                if [[ "${dry_run}" == "false" ]]; then
                    "${cmd_bld[@]}" || {
                        echo_err \
                            "serial download worker failed for accession" \
                            "'${list_job_acc[i]}'."
                        return 1
                    }
                fi
