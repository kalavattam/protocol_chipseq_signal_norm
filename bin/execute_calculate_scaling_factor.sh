...
Expected globals
----------------
  scr_sub, fil_out, tbl_met, cfg_met : file
    Submission-script, output-table, metadata-table, and metadata-configuration paths, respectively.

  dir_scr, dir_eo : dir
    Script and wrapper-log directories, respectively.

  env_nam, mode, method, csv_sip, csv_sin, eqn, aln_typ, nam_job : str
    Environment, mode, method, serialized spike-IP inputs, serialized spike-input inputs, equation, alignment type, and job-name values, respectively.

  threads, dp : int
    Thread count and output rounding precision, respectively.
  ...
  arr_mip : array of file
    Reconstructed main-IP input paths for indexed calls.

  arr_min : array of file
    Reconstructed main-input paths for indexed calls.

  arr_sip : array of file
    Reconstructed spike-IP input paths for indexed calls.

  arr_sin : array of file
    Reconstructed spike-input paths for indexed calls.

  arr_len_mip : array of int
    Broadcastable main-IP fragment-length overrides.

  arr_len_min : array of int
    Broadcastable main-input fragment-length overrides.

  arr_dep_mip : array of int
    Broadcastable main-IP depth overrides.

  arr_dep_min : array of int
    Broadcastable main-input depth overrides.

  arr_dep_sip : array of int
    Broadcastable spike-IP depth overrides.

  arr_dep_sin : array of int
    Broadcastable spike-input depth overrides.
...
Examples
--------
  1. Build the non-indexed submit command from validated scalar globals.
    '''bash
    build_cmd
    declare -p cmd_bld
    '''

  2. Build the first per-sample command from prepared input and override arrays.
    '''bash
    build_cmd 0
    declare -p cmd_bld
    '''
...
function build_cmd_cmb_slurm() {
    ...
    cmd_cmb_slurm=(
        sbatch
            ...
            --cpus-per-task=1
            --time="${time}"
            --output="${dir_eo}/${nam_job}_combine.%j.stdout.txt"
...
function build_cmd_hdr_slurm() {
    ...
    cmd_hdr_slurm=(
        sbatch
            ...
            --cpus-per-task=1
            --time="${time}"
            --output="${dir_eo}/${nam_job}_header.%j.stdout.txt"
...
function validate_args() {
    ...
    if [[ -z "${dir_eo}" ]]; then dir_eo="$(dirname "${fil_out}")/logs"; fi
...
function check_tools() {
    ...
    check_pgrm_path calculate_scaling_factor_spike || return 1
    check_pgrm_path parse_metadata_siqchip || return 1
