...
# -*- coding: utf-8 -*-
#
# Script: test_execute_filter_bams_cram_to_cram.sh
#
...
TEST_NAME="execute filter-bams CRAM to CRAM"
...
tmp="${TEST_DIR_TMP}/execute_filter_bams_cram_to_cram"
dir_in="${tmp}/input"
dir_out="${tmp}/out"
dir_err="${tmp}/logs"
...
log="${dir_log}/execute_filter_bams_prepare_cram_to_cram.log"
if \
    run_capture \
        "prepare execute filter-bams CRAM fixture for CRAM output" "${log}" \
        run_samtools view -C -T "${ref_fa}" -o "${in_cram}" "${in_sam}" \
    && run_capture \
        "index execute filter-bams CRAM fixture for CRAM output" "${log}.index" \
...
function run_case_filter() {
    ...
    if \
        run_capture \
            "execute filter-bams CRAM to CRAM ${nam_case}" "${log_lcl}" \
            "${TEST_BASH}" "${ROOT_REPO}/scripts/execute_filter_bams.sh" \
                ...
                --ref_fa "${ref_fa}" \
                --err_out "${dir_err}" \
                --nam_job "test_execute_filter_bams_cram_to_cram_${nam_case}" \
                --max_job 1 \
                "$@"
    then
        rec_pass "execute_filter_bams.sh CRAM to CRAM retain=${retain} ${nam_case} exits 0"
    else
        rec_fail \
            "execute_filter_bams.sh CRAM to CRAM retain=${retain} ${nam_case} failed; see" \
            "$(rec_relpath "${log_lcl}")"
    fi
}
...
log="${dir_log}/execute_filter_bams_cram_to_cram_sc.log"
...
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sc_ser.stdout.txt" \
    "execute CRAM-to-CRAM retain=sc serial stdout log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sc_ser.stderr.txt" \
    "execute CRAM-to-CRAM retain=sc serial stderr log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sc.filter_sc_sp.stdout.txt" \
    "execute CRAM-to-CRAM retain=sc submit stdout log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sc.filter_sc_sp.stderr.txt" \
    "execute CRAM-to-CRAM retain=sc submit stderr log"

if [[ -s "${outfile}" ]]; then
    if \
        run_capture \
            "quickcheck execute filter-bams CRAM to CRAM sc" \
            "${dir_log}/execute_filter_bams_cram_to_cram_sc_quickcheck.log" \
...
log="${dir_log}/execute_filter_bams_cram_to_cram_sp.log"
...
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sp_ser.stdout.txt" \
    "execute CRAM-to-CRAM retain=sp serial stdout log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sp_ser.stderr.txt" \
    "execute CRAM-to-CRAM retain=sp serial stderr log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sp.filter_sc_sp.stdout.txt" \
    "execute CRAM-to-CRAM retain=sp submit stdout log"
assert_file_exists \
    "${dir_err}/test_execute_filter_bams_cram_to_cram_sp.filter_sc_sp.stderr.txt" \
    "execute CRAM-to-CRAM retain=sp submit stderr log"

if [[ -s "${outfile}" ]]; then
    if \
        run_capture \
            "quickcheck execute filter-bams CRAM to CRAM sp" \
            "${dir_log}/execute_filter_bams_cram_to_cram_sp_quickcheck.log" \
            run_samtools quickcheck "${outfile}"
    then
