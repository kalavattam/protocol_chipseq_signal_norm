...
if ! \
    prepare_filter_alignments_bam_fixture \
        "${in_sam}" "${in_bam}" "${log}" "submit filter-alignments BAM fixture"
then
    finish
    exit $?
fi
...
#  Run submit_filter_alignments.sh for one retain mode
function run_case_filter() {
    ...
    if \
        run_capture \
            "submit filter-alignments ${nam_case}" "${log_lcl}" \
            "${TEST_BASH}" "${ROOT_REPO}/scripts/submit_filter_alignments.sh" \
                ...
                --retain "${retain}" \
                --err_out "${dir_err}" \
                --nam_job "test_submit_filter_alignments_${nam_case}" \
                "$@"
    then
        rec_pass "submit_filter_alignments.sh retain=${retain} ${nam_case} exits 0"
    else
        rec_fail \
            "submit_filter_alignments.sh retain=${retain} ${nam_case} failed; see" \
            "$(rec_relpath "${log_lcl}")"
    fi
}
...
if [[ -s "${outfile}" ]]; then
    run_capture \
        "idxstats submit filter-alignments sc" "${idxstats}" \
...
if [[ -s "${outfile}" ]]; then
    run_capture \
        "idxstats submit filter-alignments sp" "${idxstats}" \
        run_samtools idxstats "${outfile}"
