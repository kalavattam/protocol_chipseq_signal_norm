...
if \
    run_capture \
        "submit align-fastqs bwa-mem2 mem se" "${log}" \
...
then
    rec_pass "submit_align_fastqs.sh bwa-mem2 MEM SE BAM exits 0"
else
    rec_fail \
...
assert_file_nonempty "${outfile_se}" "submit bwa-mem2 MEM SE BAM output"
assert_file_nonempty "${outfile_se}.bai" "submit bwa-mem2 MEM SE BAM index"
...
if [[ -s "${outfile_se}" ]]; then
    ...
    then
        rec_pass "submit bwa-mem2 MEM SE BAM passes samtools quickcheck"
    else
        rec_fail "submit bwa-mem2 MEM SE BAM fails samtools quickcheck"
    fi

    run_capture \
        "idxstats submit align-fastqs bwa-mem2 MEM SE BAM" "${idxstats_se}" \
        run_samtools idxstats "${outfile_se}"
    run_capture \
        "view submit align-fastqs bwa-mem2 MEM SE BAM" "${view_se}" \
...
if \
    run_capture \
        "submit align-fastqs bwa-mem2 mem pe" "${log}" \
        "${TEST_BASH}" "${ROOT_REPO}/scripts/submit_align_fastqs.sh" \
            --env_nam "${env_nam}" \
            --dir_scr "${ROOT_REPO}/scripts" \
            --threads 1 \
...
then
    rec_pass "submit_align_fastqs.sh bwa-mem2 MEM PE BAM exits 0"
else
    rec_fail \
...
assert_file_nonempty "${outfile_pe}" "submit bwa-mem2 MEM PE BAM output"
assert_file_nonempty "${outfile_pe}.bai" "submit bwa-mem2 MEM PE BAM index"
...
if [[ -s "${outfile_pe}" ]]; then
    ...
    then
        rec_pass "submit bwa-mem2 MEM PE BAM passes samtools quickcheck"
    else
        rec_fail "submit bwa-mem2 MEM PE BAM fails samtools quickcheck"
    fi

    run_capture \
        "idxstats submit align-fastqs bwa-mem2 MEM PE BAM" "${idxstats_pe}" \
        run_samtools idxstats "${outfile_pe}"
    run_capture \
        "view submit align-fastqs bwa-mem2 MEM PE BAM" "${view_pe}" \
