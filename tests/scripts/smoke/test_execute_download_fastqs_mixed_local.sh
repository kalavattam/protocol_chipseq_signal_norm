...
log_server="${dir_log}/execute_download_fastqs_mixed_local_http.log"
...
if \
    run_capture \
        "execute download-fastqs mixed local HTTP wet run" "${log_run}" \
...
then
    rec_pass "execute_download_fastqs.sh mixed local HTTP wet run exits 0"
else
    rec_fail \
...
assert_custom_symlink \
    "${symlink_se}" "execute download-fastqs mixed SE custom FASTQ"
assert_custom_symlink \
    "${symlink_r1}" "execute download-fastqs mixed PE R1 custom FASTQ"
assert_custom_symlink \
    "${symlink_r2}" "execute download-fastqs mixed PE R2 custom FASTQ"
