...
#+ inputs
function run_case_filter_parallel() {
    local retain="${1:-}"
    local nam_job="${2:-}"
    local dir_out_lcl="${3:-}"
    local dir_err_lcl="${4:-}"
...
function assert_parallel_output_sp() {
    local sample="${1:-}"
    local fil_out="${dir_out_sp}/${sample}.sp.cram"
    local lbl_quickcheck=""
    local log_quickcheck=""
    local log_stderr="${dir_err_sp}/${nam_job_sp}.${sample}.stderr.txt"
    local log_stdout="${dir_err_sp}/${nam_job_sp}.${sample}.stdout.txt"
    local out_chrun="${dir_out_sp}/${sample}.chrUn.count.txt"
    local out_i="${dir_out_sp}/${sample}.I.count.txt"
    local out_sp_i="${dir_out_sp}/${sample}.SP_I.count.txt"
    local out_sp_ii_tg="${dir_out_sp}/${sample}.SP_II_TG.count.txt"
    local out_sp_mito="${dir_out_sp}/${sample}.SP_Mito.count.txt"
    local out_sp_mtr="${dir_out_sp}/${sample}.SP_MTR.count.txt"

    lbl_quickcheck="quickcheck execute filter-alignments GNU Parallel "
    lbl_quickcheck+="retain=sp ${sample}"

    log_quickcheck="${dir_log}/execute_filter_alignments_cram_parallel_sp_"
    log_quickcheck+="${sample}_quickcheck.log"
...
function assert_parallel_output_sc() {
    local sample="${1:-}"
    local fil_out="${dir_out_sc}/${sample}.sc.cram"
    local lbl_quickcheck=""
    local log_quickcheck=""
    local log_stderr="${dir_err_sc}/${nam_job_sc}.${sample}.stderr.txt"
    local log_stdout="${dir_err_sc}/${nam_job_sc}.${sample}.stdout.txt"
    local out_chrun="${dir_out_sc}/${sample}.chrUn.count.txt"
    local out_i="${dir_out_sc}/${sample}.I.count.txt"
    local out_mito="${dir_out_sc}/${sample}.Mito.count.txt"
    local out_sp_i="${dir_out_sc}/${sample}.SP_I.count.txt"

    lbl_quickcheck="quickcheck execute filter-alignments GNU Parallel "
    lbl_quickcheck+="retain=sc ${sample}"

    log_quickcheck="${dir_log}/execute_filter_alignments_cram_parallel_sc_"
    log_quickcheck+="${sample}_quickcheck.log"
...
log_prep_1="${dir_log}/execute_filter_alignments_cram_parallel_prepare_1.log"
log_prep_2="${dir_log}/execute_filter_alignments_cram_parallel_prepare_2.log"
log_sc="${dir_log}/execute_filter_alignments_cram_parallel_sc.log"
log_sp="${dir_log}/execute_filter_alignments_cram_parallel_sp.log"
log_env_parallel="${dir_log}/execute_filter_alignments_cram_parallel_env.log"
...
assert_parallel_output_sp "filter_sc_sp_parallel_1"
assert_parallel_output_sp "filter_sc_sp_parallel_2"
...
assert_parallel_output_sc "filter_sc_sp_parallel_1"
assert_parallel_output_sc "filter_sc_sp_parallel_2"
