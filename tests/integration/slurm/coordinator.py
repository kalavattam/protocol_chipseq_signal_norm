...
Prepare and inspect a fixed run without contacting a remote host::
    ...
    python tests/integration/slurm/coordinator.py prepare --run_id wet-20260716 \
...
Render transfer and remote commands without running them::
    ...
    python tests/integration/slurm/coordinator.py push --run_id wet-20260716 --dry_run
    python tests/integration/slurm/coordinator.py instructions --run_id wet-20260716
"""
...
def validate_remote_token(value: str, label: str, *, empty_ok: bool = False) -> str:
...
def safe_remote_run_dir(remote_root: str, run_id: str) -> PurePosixPath:
    ...
    if not remote_root or root == PurePosixPath("/") or ".." in root.parts:
        raise BundleError("remote run root must be a non-root path without '..'")
...
def validate_remote_python(value: str) -> str:
    ...
    if not path.is_absolute() or ".." in path.parts:
        raise BundleError("remote Python must be an absolute path without '..'")
...
def source_entry(
    root: Path, relative: PurePosixPath, tracked: set[PurePosixPath]
) -> dict[str, Any]:
    ...
    return {
        ...
        "git_state": "tracked" if relative in tracked else "required_untracked",
    }
...
def create_source_archive(
    ...
    with archive_path.open("wb") as raw_handle, gzip.GzipFile(
        filename="", fileobj=raw_handle, mode="wb", mtime=0
    ) as gz_handle, tarfile.open(fileobj=gz_handle, mode="w") as archive:
        add_tar_bytes(
            archive,
...
def safe_extract_archive(archive_path: Path, destination: Path) -> Path:
    ...
    if destination.exists():
        raise BundleError(f"extraction destination already exists: {destination}")
...
def verify_source_tree(source_root: Path, manifest: dict[str, Any]) -> None:
    ...
    if actual != set(expected):
        ...
        raise BundleError(f"source membership mismatch: missing={missing}, extra={extra}")
...
def prepared_session(bundle_dir: Path, run_id: str) -> Path:
    ...
    if candidate.parent != base:
        raise BundleError("local session path escaped the configured bundle directory")
    return candidate
...
def session_config(session: Path) -> dict[str, Any]:
    ...
    if config.get("run_id") != session.name:
        raise BundleError("prepared config run ID does not match session directory")
    validate_run_id(str(config.get("run_id", "")))
    return config


def build_remote_config(args: argparse.Namespace, digest: str) -> dict[str, Any]:
...
def prepare(args: argparse.Namespace) -> int:
    ...
    launcher_package = (
        launcher_root / "src" / "protocol_chipseq_signal_norm"
    )
    ...
    write_json(incoming / "transfer_manifest.json", transfer_manifest(incoming))
    state = {
        "run_id": args.run_id,
        "created_at": created_at,
        "session": str(session),
...
def ssh_target(config: dict[str, Any]) -> str:
    ...
    user = validate_remote_token(
        str(config.get("ssh_user", "")), "SSH user", empty_ok=True
    )
...
def remote_launch_command(config: dict[str, Any]) -> str:
    ...
    launcher = (
        incoming / "launcher" / "tests" / "integration" / "slurm"
        / "coordinator.py"
    )
    cfg = incoming / "remote_config.json"
    archive = incoming / "source.tar.gz"
    command = [
    ...
    return (
        "PYTHONDONTWRITEBYTECODE=1 RUN_SLURM=1 WAIT_SLURM=1 "
        "CONFIRM_SLURM_WET=1 "
        + shlex.join(command)
    )
...
def instructions(args: argparse.Namespace) -> int:
    ...
    print(
        shlex.join(
            [
                str(config["remote_python"]),
                (incoming / "launcher/tests/integration/slurm/coordinator.py").as_posix(),
                "status",
                "--session_dir",
...
def pull_command(
    session: Path, config: dict[str, Any], destination: Path
) -> list[str]:
...
def pull(args: argparse.Namespace) -> int:
    ...
    config = session_config(session)
    destination = Path(
        args.result_destination or config["result_destination"]
    )
    ...
    if result.returncode:
        raise BundleError(
            f"result pull failed ({result.returncode}): {result.stderr.strip()}"
        )
...
def verify_results(
    ...
    preflight = load_json(result_dir / "preflight.json")
    run_manifest = load_json(result_dir / "run_manifest.json")
    jobs_payload = load_json(result_dir / "jobs.json")
    ...
    for label, payload in (
        ...
        if not isinstance(payload, dict):
            raise BundleError(f"{label} result must be a JSON object")
        if payload.get("run_id") != run_id:
            raise BundleError(f"{label} run ID mismatch")
    if run_manifest.get("source_manifest_sha256") != source_manifest_sha256:
        raise BundleError("result source manifest does not match prepared bundle")
    ...
    if len(by_key) != len(jobs) or set(by_key) != set(required_ids):
        raise BundleError("a required job is missing, duplicated, or silently omitted")
    ...
    for key in required_ids:
        ...
        if bool(job.get("success")) != job_ok:
            raise BundleError(f"job success field disagrees with evidence: {key}")
    ...
    if summary.get("required_job_count") != len(required_ids):
    ...
    try:
        exit_status = int((result_dir / "exit_status.txt").read_text().strip())
...
def status(args: argparse.Namespace) -> int:
    ...
    if args.remote:
        ...
        command = ["ssh", ssh_target(config), "cat", "--", remote_status.as_posix()]
        if args.dry_run:
            print_commands([command])
            return 0
        result = run_command(command)
...
def verify_transfer(incoming: Path) -> None:
    ...
    for relative, digest in expected.items():
        if not SHA256_RE.fullmatch(digest) or sha256_file(
            incoming / relative
        ) != digest:
            raise BundleError(f"incoming transfer checksum mismatch: {relative}")
...
def remote_launch(args: argparse.Namespace) -> int:
    ...
    if sha256_file(source_manifest) != sha256_file(prepared_manifest):
        raise BundleError("extracted source manifest differs from prepared manifest")
    ...
    runner = source_root / "tests" / "integration" / "slurm" / "run_wet_tests.sh"
    ...
    bundled_src = str(source_root / "src")
    environment["PYTHONPATH"] = (
        bundled_src + os.pathsep + environment["PYTHONPATH"]
...
def validate_resource_bounds(resources: dict[str, Any]) -> None:
    ...
    if not 1 <= poll <= 30 or not 60 <= wait <= 900:
        raise BundleError("wet polling or wait bound is outside the safe range")
...
def run_preflight(config: dict[str, Any]) -> dict[str, Any]:
    ...
    env_ok = env_check.returncode == 0 and re.search(
        rf"(^|\s){re.escape(env_name)}(\s|$)", env_check.stdout, re.MULTILINE
    ) is not None
    checks = {
        "python_version": python_ok,
        "bash_version": bash_ok,
        "scheduler_commands": all(item["available"] for item in commands.values()),
        "environment": env_ok,
    ...
    return [
        {
            ...
            "arguments": [
                "--env_nam", str(config["environment_name"]),
                "--dir_scr", str(source / "bin"),
                "--threads", "1",
                "--aligner", "bowtie2",
                "--bt2_mode", "global",
                "--mapq", "0",
                "--index", str(fixtures / "bowtie2" / "tiny"),
                "--csv_fil_in", str(
                    source / "tests/fixtures/slurm/fastq/tiny_se.fastq"
                ),
                "--dir_out", str(align_art),
                "--out_ext", "bam",
                "--sfx_se", ".fastq",
                "--sfx_pe", "_R1.fastq",
                "--dir_eo", str(align_art / "logs"),
                "--nam_job", f"wet7i-align-worker-{suffix}",
            ],
            "output_directories": [align_art, align_art / "logs"],
            "assertions": [
                {"kind": "nonempty", "path": str(align_art / "tiny_se.bam")},
                {"kind": "nonempty", "path": str(align_art / "tiny_se.bam.bai")},
                {
                    "kind": "command",
                    "command": [
                        provider, "run", "-n", str(config["environment_name"]),
                        "samtools", "quickcheck", str(align_art / "tiny_se.bam"),
                    ],
                ...
                },
                {
                    ...
                    "command": [
                        provider, "run", "-n", str(config["environment_name"]),
                        "samtools", "view", str(align_art / "tiny_se.bam"),
                    ],
                    "text": "tiny_se_read_1\t",
                },
            ],
        },
        {
            ...
            "arguments": [
                ...
                "--mode", "signal",
                "--method", "unadj",
                "--csv_fil_in", str(fixtures / "tiny_signal.bam"),
                "--chr_sizes", str(fixtures / "tiny.fa.fai"),
                "--csv_fil_out", str(signal_art / "tiny_se_signal_unadj.bdg"),
                "--csv_usr_frg", "NA",
                "--csv_scl_fct", "NA",
                "--siz_bin", "10",
                "--engine", "window",
                "--dp", "3",
                "--dir_eo", str(signal_art / "logs"),
                "--nam_job", f"wet7i-signal-worker-{suffix}",
            ],
            "output_directories": [signal_art, signal_art / "logs"],
            "assertions": [
                {"kind": "nonempty", "path": str(signal_art / "tiny_se_signal_unadj.bdg")},
                {"kind": "contains", "path": str(signal_art / "tiny_se_signal_unadj.bdg"), "text": "I\t0\t10\t10"},
                {"kind": "contains", "path": str(signal_art / "tiny_se_signal_unadj.bdg"), "text": "I\t20\t30\t10"},
            ],
        },
    ]
...
def prepare_remote_fixtures(
    config: dict[str, Any], commands: list[str]
) -> tuple[bool, str]:
    ...
    command_list = [
        [provider, "run", "-n", environment, "bowtie2-build", str(reference), str(index)],
        [provider, "run", "-n", environment, "samtools", "faidx", str(reference)],
        [
            provider,
            "run",
            "-n",
    ...
    expected = [
        ...
        *[Path(f"{index}{suffix}") for suffix in (
...
def sbatch_command(
    config: dict[str, Any], job: dict[str, Any], script: Path
) -> list[str]:
...
def evaluate_assertion(
    spec: dict[str, Any], commands: list[str] | None = None
) -> dict[str, Any]:
    ...
    if kind == "nonempty":
        ...
        if not isinstance(command, list) or not all(
            ...
            if kind == "command_contains":
                passed = passed and str(spec.get("text", "")) in completed.stdout
            detail = (
...
def finalize_results(
    ...
    result = Path(config["result_dir"])
    success = preflight.get("success") is True and len(jobs) == len(required_keys)
    ...
    summary = {
        ...
        "failed_job_keys": [job["job_key"] for job in jobs if not job.get("success")],
    ...
    write_json(result / "jobs.json", {"run_id": config["run_id"], "jobs": jobs})
    ...
    (result / "exit_status.txt").write_text(f"{status_value}\n", encoding="utf-8")
...
def wet_run(args: argparse.Namespace) -> int:
    ...
    for gate in ("RUN_SLURM", "WAIT_SLURM", "CONFIRM_SLURM_WET"):
        if os.environ.get(gate) != "1":
            raise BundleError(f"{gate}=1 is required before any wet Slurm action")
    ...
    if not preflight["success"]:
        return finalize_results(config, preflight, jobs, commands, required_keys)
    write_json(result / "live_status.json", {"run_id": run_id, "state": "submitting"})
    for definition in definitions:
            ...
            jobs.append(
                {
                    ...
                    "assertions": [
                        {"kind": "sbatch_test_only", "passed": False, "detail": test_result.stderr.strip()}
                    ],
            ...
            jobs.append(
                {
                    ...
                    "stderr_path": f"stderr/{job_key}.err",
                    "assertions": [
                        {"kind": "sbatch_acceptance", "passed": False, "detail": submitted.stderr.strip()}
                    ],
                    "cleanup_result": "passed",
                    "success": False,
                }
        ...
        while time.monotonic() < deadline:
            ...
            if state_row and state_row["state"].split("+")[0] in TERMINAL_STATES:
        ...
        record["assertions"] = [
            evaluate_assertion(spec, commands) for spec in definition["assertions"]
        ]
...
def add_common_session_args(parser: CapArgumentParser) -> None:
    ...
    parser.add_argument(
        "-ri", "--run_id", dest="run_id", required=True,
        help="Portable prepared run ID.",
    )
    parser.add_argument(
        "-bd", "--bundle_dir", dest="bundle_dir",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    prep.add_argument("-rt", "--root", dest="root", default=str(repo_root_from_script()), help="Public repository root.")
    prep.add_argument("-sc", "--scope", dest="scope", default=DEFAULT_SCOPE, help="Declared validation scope.")
    prep.add_argument("-sh", "--ssh_host", dest="ssh_host", required=True, help="SSH host alias or name.")
    prep.add_argument("-su", "--ssh_user", dest="ssh_user", default="", help="SSH user (default: current SSH configuration).")
    prep.add_argument("-rr", "--remote_run_root", dest="remote_run_root", default=DEFAULT_REMOTE_ROOT.as_posix(), help=f"Isolated remote run root (default: {DEFAULT_REMOTE_ROOT}).")
    prep.add_argument("-pt", "--partition", dest="partition", required=True, help="Slurm partition.")
    prep.add_argument("-ac", "--account", dest="account", required=True, help="Slurm account.")
    prep.add_argument("-en", "--env_name", dest="env_name", default="env_protocol", help="Conda environment name (default: env_protocol).")
    prep.add_argument("-rp", "--remote_python", dest="remote_python", required=True, help="Absolute remote Python executable path.")
    prep.add_argument("-rd", "--result_destination", dest="result_destination", default=str(DEFAULT_RESULT_DIR), help="Local pulled-result base directory.")
    prep.add_argument("-mm", "--memory", dest="memory", default="1G", help="Per-job memory, at most 1G (default: 1G).")
    prep.add_argument("-tl", "--time_limit", dest="time_limit", default="00:05:00", help="Per-job wall time, at most five minutes.")
    prep.add_argument("-ps", "--poll_seconds", dest="poll_seconds", type=int, default=5, help="Scheduler poll interval (default: 5).")
    prep.add_argument("-ws", "--wait_seconds", dest="wait_seconds", type=int, default=600, help="Terminal-state wait bound (default: 600).")
    prep.add_argument("-dr", "--dry_run", dest="dry_run", action="store_true", default=False, help="Run script in dry-run mode. Inventory without writing artifacts.")
    ...
    for name, handler in (("push", push), ("pull", pull)):
        ...
        child.add_argument("-dr", "--dry_run", dest="dry_run", action="store_true", default=False, help="Run script in dry-run mode. Print commands without network access.")
        if name == "pull":
            child.add_argument("-rd", "--result_destination", dest="result_destination", default="", help="Override local pulled-result base directory.")
    ...
    verify_parser.add_argument("-rd", "--result_destination", dest="result_destination", default="", help="Override local pulled-result base directory.")
    ...
    status_group.add_argument("-ri", "--run_id", dest="run_id", default=None, help="Prepared run ID.")
    status_group.add_argument("-sd", "--session_dir", dest="session_dir", default=None, help="Explicit local or remote session directory.")
    status_parser.add_argument("-bd", "--bundle_dir", dest="bundle_dir", default=str(DEFAULT_BUNDLE_DIR), help="Local session base directory.")
    status_parser.add_argument("-rm", "--remote", dest="remote", action="store_true", default=False, help="Inspect through SSH; never the default.")
    status_parser.add_argument("-dr", "--dry_run", dest="dry_run", action="store_true", default=False, help="Run script in dry-run mode. Print the remote status command only.")
    ...
    clean_parser.add_argument("-dr", "--dry_run", dest="dry_run", action="store_true", default=False, help="Run script in dry-run mode. Show confined removal without deleting.")
    ...
    remote_parser.add_argument("-cf", "--config", dest="config", required=True, help="Transferred remote configuration file.")
    remote_parser.add_argument("-ar", "--archive", dest="archive", required=True, help="Transferred source archive.")
    ...
    wet_parser.add_argument("-cf", "--config", dest="config", required=True, help="Extracted runtime configuration file.")
