...
class TemporaryRepository:
    ...
    def create(self) -> None:
        ...
        required = self.root / "src/protocol_chipseq_signal_norm/required_new.py"
...
def write_result_bundle(
    ...
    bundle.write_json(root / "preflight.json", {"run_id": run_id, "success": True})
    bundle.write_json(
    ...
    success = len(jobs) == len(required) and all(job["success"] for job in jobs)
    bundle.write_json(
        root / "summary.json",
        {
            "run_id": run_id,
...
class SourceBundleTests(unittest.TestCase):
    ...
    def test_manifest_is_deterministic_and_includes_dirty_untracked_files(self) -> None:
        first = self.manifest()
        ...
        self.assertIn(
            "src/protocol_chipseq_signal_norm/required_new.py", paths
        )
        tracked = next(
    ...
    def test_manifest_excludes_ignored_outputs_caches_reports_and_metadata(self) -> None:
    ...
    def test_archive_bytes_and_checksums_are_deterministic(self) -> None:
        ...
        digest_1 = bundle.create_source_archive(self.repo.root, first, manifest)
        digest_2 = bundle.create_source_archive(self.repo.root, second, manifest)
        ...
        self.assertIn(
            "# dirty", (source / "bin" / "needed.sh").read_text()
        )
    ...
    def test_run_id_and_remote_root_validation(self) -> None:
        ...
        for root in ("/", "../runs", "runs/../other", "runs with spaces", "x;id", ""):
            with self.subTest(root=root), self.assertRaises(bundle.BundleError):
...
class InterfaceSafetyTests(unittest.TestCase):
    ...
    def test_prepare_dry_run_writes_nothing(self) -> None:
        ...
        args = argparse.Namespace(
            root=str(repo.root), run_id="dry-1", scope="dry scope",
            created_at="2026-07-16T00:00:00Z", bundle_dir=str(bundle_dir),
            dry_run=True,
        )
    ...
    def test_push_pull_and_remote_launch_commands_are_isolated(self) -> None:
        ...
        launch = bundle.remote_launch_command(
            {**config, "run_id": "wet-1"}
        )
        self.assertTrue(
            launch.startswith(
                "PYTHONDONTWRITEBYTECODE=1 RUN_SLURM=1 WAIT_SLURM=1 "
                "CONFIRM_SLURM_WET=1 /opt/env/bin/python3 "
            )
        )
    ...
    def test_clean_requires_marker_and_remains_confined(self) -> None:
        ...
        args = argparse.Namespace(bundle_dir=str(base), run_id="wet-1", dry_run=False)
        with self.assertRaises(bundle.BundleError):
            bundle.clean(args)
        bundle.write_json(
...
class ResultVerifierTests(unittest.TestCase):
    ...
    def test_missing_and_failed_jobs_are_rejected(self) -> None:
        ...
        write_result_bundle(
            missing, self.run_id, self.digest,
            [result_job("align_bowtie2_se")], required,
        )
    ...
    def test_malformed_result_json_and_run_id_mismatch_are_rejected(self) -> None:
...
class FakeSlurmTests(unittest.TestCase):
    ...
    def runtime_config(self, name: str) -> tuple[dict[str, object], Path]:
        ...
        config: dict[str, object] = {
            ...
            "source_manifest_sha256": bundle.sha256_file(source.parent / "manifest.json"),
            "partition": "test-partition",
            "account": "test-account",
            "environment_name": "env_protocol",
            "declared_validation_scope": "fake wet scope",
            "resources": {
                "job_count": 2, "cpus_per_task": 1, "memory": "256M",
                "time_limit": "00:01:00", "poll_seconds": 1,
                "wait_seconds": 60,
            },
        }
    ...
    def definitions(self, config: dict[str, object]) -> list[dict[str, object]]:
        ...
        return [
            {
                ...
                "output_directories": [
                    ...
                    Path(str(config["result_dir"])) / "artifacts" / key / "logs",
                ],
            }
    ...
    def test_real_job_definitions_declare_each_worker_output_directory(
        ...
        for definition in definitions:
            ...
            } if "--dir_out" in arguments else {
                Path(arguments[arguments.index("--csv_fil_out") + 1]).parent,
                Path(arguments[arguments.index("--dir_eo") + 1]),
            }
            self.assertEqual(set(definition["output_directories"]), configured)

    def test_worker_output_directories_exist_before_sbatch_test_only(self) -> None:
    ...
    def test_fake_slurm_success_records_both_terminal_jobs(self) -> None:
        ...
        with mock.patch.dict(os.environ, self.wet_environment(), clear=True), mock.patch.object(
            bundle, "job_definitions", return_value=self.definitions(config)
        ), mock.patch.object(
            bundle,
        ...
        jobs = bundle.load_json(Path(str(config["result_dir"])) / "jobs.json")["jobs"]
        self.assertEqual([job["final_state"] for job in jobs], ["COMPLETED"] * 2)
    ...
    def test_fake_failed_job_is_not_submission_success(self) -> None:
        ...
        with mock.patch.dict(
            os.environ, self.wet_environment("FAILED"), clear=True
        ), mock.patch.object(
            bundle, "job_definitions", return_value=self.definitions(config)
        ), mock.patch.object(
            bundle,
        ...
        summary = bundle.load_json(Path(str(config["result_dir"])) / "summary.json")
    ...
    def test_retired_slurm_wait_spelling_cannot_enable_submission(self) -> None:
    ...
    def test_shell_runner_missing_confirmation_never_calls_fake_sbatch(self) -> None:
