...
def test_parse_skp_pfx_special_cases():
...
def test_is_header_and_read_data_line_skip_metadata():
...
def test_open_in_and_open_out_handle_gzip(tmp_path):
    path = tmp_path / "out.txt.gz"

    with open_out(str(path)) as fh:
        fh.write("hello\n")

    with gzip.open(path, "rt", encoding="utf-8") as fh:
        assert fh.read() == "hello\n"
    with open_in(str(path)) as fh:
        assert fh.read() == "hello\n"


def test_ensure_single_stdin_rejects_multiple_stdin_paths():
