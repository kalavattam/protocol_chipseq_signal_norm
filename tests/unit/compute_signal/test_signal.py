...
def collect_indexed_windows(
    ...
    for chrom, chrom_size in chrom_sizes.items():
        for start in range(0, chrom_size, window_size):
            fragments.extend(iter_indexed_fragments(
                str(fil_in),
                chrom=chrom,
...
def test_calc_sig_chrom_accumulates_fragment_overlap_by_bin():
...
def test_calc_sig_chrom_length_and_count_normalization():
    observed = calc_sig_chrom(
        "chrI",
        [(0, 10, 10), (5, 15, 10)],
        frg_tot=2,
...
def test_calc_sig_chrom_array_emits_only_touched_bins():
    fragments = [
        (0, 26040, 3519),
        (26080, 40000, 3519),
    ]
...
def test_calc_sig_chrom_rejects_invalid_bin_size():
    with pytest.raises(ValueError, match="siz_bin"):
        calc_sig_chrom("chrI", [], 0, 0, False, False)


def test_resolve_chrom_sizes_rejects_conflicts(tmp_path):
...
def test_compute_signal_engines_match_and_clamp_bedgraph_end(tmp_path):
    ...
    assert main([
        "--fil_in", str(fil_in),
        "--fil_out", str(out_chrom),
        "--method", "unadj",
        "--siz_bin", "10",
    ]) == 0
    assert main([
        "--fil_in", str(fil_in),
        "--fil_out", str(out_window),
        "--method", "unadj",
        "--siz_bin", "10",
        "--engine", "window",
        "--indexed_window_size", "15",
    ]) == 0
...
@pytest.mark.parametrize("subdir,usr_frg", [
...
def test_indexed_chrom_and_window_fetch_match_serial_fragments(
    ...
    serial = sorted(iter_bam_fragments(
        str(fil_in),
        chrom_sizes=chrom_sizes,
        usr_frg=usr_frg,
    ))
    ...
    for chrom in chrom_sizes:
        indexed_chrom.extend(iter_indexed_fragments(
            str(fil_in),
            chrom=chrom,
            chrom_sizes=chrom_sizes,
            usr_frg=usr_frg,
...
@pytest.mark.parametrize("subdir,ref_args,usr_frg", [
    ("bam/se/tiny_se.bam", [], "30"),
    (
...
def test_compute_signal_public_engines_match_default(
    ...
    assert main([*base_args, "--fil_out", str(out_default)]) == 0
    assert main([
        *base_args,
        "--fil_out", str(out_chrom),
        "--engine", "chrom",
...
@pytest.mark.parametrize("result_format", [
...
def test_compute_signal_hidden_sparse_variants_match_public_baseline(
    ...
    assert main([
        *base_args,
        "--fil_out", str(out_variant),
        "--engine", "window",
        "--indexed_window_size", "15",
        "--prototype_result_format", result_format,
    ]) == 0

    assert read_text(out_variant) == read_text(out_default)


@pytest.mark.parametrize("bed_strategy", [
    "indexed_chrom",
    "indexed_window",
])
def test_compute_signal_hidden_indexed_bed_strategies_match_serial(
    ...
    assert main([
        "--fil_in", str(fil_in),
        "--fil_out", str(out_serial),
        "--prototype_bed_strategy", "serial",
    ]) == 0
    assert main([
        "--fil_in", str(fil_in),
        "--fil_out", str(out_indexed),
        "--threads", "1",
        "--prototype_bed_strategy", bed_strategy,
        "--indexed_window_size", "15",
    ]) == 0
...
def test_compute_signal_auto_bed_falls_back_without_index(tmp_path):
    ...
    assert main([
        ...
        "--fil_out", str(out_bed),
        "--threads", "2",
        "--prototype_bed_strategy", "auto",
    ]) == 0
    assert out_bed.read_text(encoding="utf-8")


def test_compute_signal_public_engines_require_index(tmp_path):
    fil_in = tmp_path / "tiny_no_index.bam"
    shutil.copyfile(FIXTURES / "bam" / "se" / "tiny_se.bam", fil_in)
    ...
    with pytest.raises(SystemExit, match="alignment index"):
        main([
            "--fil_in", str(fil_in),
            "--fil_out", str(tmp_path / "out.bdg"),
            "--method", "unadj",
            "--engine", "chrom",
        ])


def test_compute_signal_public_cram_requires_reference_fasta(tmp_path):
...
def test_compute_signal_public_engines_and_bed_output(tmp_path):
    fil_in = FIXTURES / "bam" / "pe" / "tiny_pe.bam"
    ...
    for engine in ("chunk", "indexed_chrom"):
        with pytest.raises(SystemExit):
            main([
                "--fil_in", str(fil_in),
                "--fil_out", str(tmp_path / f"{engine}.bdg"),
                "--method", "unadj",
                "--engine", engine,
            ])
...
def test_compute_signal_rejects_dash_io(tmp_path):
    fil_in = FIXTURES / "bam" / "se" / "tiny_se.bam"
