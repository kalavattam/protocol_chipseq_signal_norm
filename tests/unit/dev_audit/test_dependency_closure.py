...
class DependencyClosurePackageTest(unittest.TestCase):
    ...
    def make_tracked_shell(self, source: str) -> tuple[tempfile.TemporaryDirectory, Path]:
        ...
        subprocess.run(["git", "-C", str(root), "add", "sample.sh"], check=True)
    ...
    def valid_unit_coverage(self) -> dict[str, object]:
        return {
            ...
            "changed_blocks": [{"represented": True, "exact_diff_retained": True}],
    ...
    def load_child(self, child: str) -> dict[str, object]:
        ...
        selection = load_target_selection(
            SimpleNamespace(
                path_values=None,
                paths_from=Path("dev/config/pilots/download_fastqs_dependency_closure.json"),
                package_child=child,
    ...
    def load_modified_linked_config(self, mutate: object) -> dict[str, object]:
        """Load a temporary external-scratch mutation of the linked config."""

        root = Path(__file__).resolve().parents[3]
        source = root / "dev/config/pilots/download_fastqs_dependency_closure.json"
        ...
        with tempfile.NamedTemporaryFile(
            ...
            return load_target_selection(
                SimpleNamespace(
                    ...
                    package_child="download-fastqs-dependency-closure-runtime-production",
                    umbrella_run_id="linked-test-run-group",
                ),
                root,
            )

    def test_nested_standalone_group_closure_does_not_end_function(self) -> None:
        from generate_prompts import _shell_source_units
...
"""
        ...
        function = next(unit for unit in units if unit["unit_kind"] == "shell_function")
        self.assertIn("echo after_group", function["source"])
        self.assertEqual(function["end_line"], 6)

    def test_multiple_nested_groups_remain_in_one_complete_function(self) -> None:
        from generate_prompts import _shell_source_units
...
"""
        functions = [unit for unit in _shell_source_units("sample.sh", source) if unit["unit_kind"] == "shell_function"]
...
"""
        units = _shell_source_units("sample.sh", source)
        function = next(unit for unit in units if unit["unit_kind"] == "shell_function")
        top_level = next(unit for unit in units if unit["unit_kind"] == "top_level_region")
        self.assertNotIn('main "$@"', function["source"])
        self.assertIn('main "$@"', top_level["source"])

    def test_top_level_bootstrap_group_is_one_complete_top_level_region(self) -> None:
        from generate_prompts import _shell_source_units
...
"""
    ...
    def test_deletion_only_hunk_inside_function_keeps_before_unit(self) -> None:
        from generate_prompts import configured_semantic_units
...
"""
        ...
        (root / "sample.sh").write_text(before.replace("    echo removed_exactly\n", ""), encoding="utf-8")
        units, coverage = configured_semantic_units(root, self.synthetic_selection())
        demo = next(unit for unit in units if unit["unit_name"] == "demo")
        self.assertNotIn("before_source", demo)
        self.assertTrue(any("-    echo removed_exactly" in item for item in demo["diff_evidence"]))
        self.assertTrue(coverage["all_changed_blocks_covered"])

    def test_deletion_only_top_level_block_keeps_complete_before_region(self) -> None:
        from generate_prompts import configured_semantic_units

        before = """source ./helpers.sh || {
...
"""
        temporary, root = self.make_tracked_shell(before)
        self.addCleanup(temporary.cleanup)
        (root / "sample.sh").write_text("function main() {\n    :\n}\n", encoding="utf-8")
        units, coverage = configured_semantic_units(root, self.synthetic_selection())
        before_regions = [unit for unit in units if unit["unit_kind"] == "top_level_region" and unit.get("source_state") == "before"]
...
"""
        units = _shell_source_units("sample.sh", source)
        top_level_source = "".join(unit["source"] for unit in units if unit["unit_kind"] == "top_level_region")
        self.assertNotIn("must_stay_in_function", top_level_source)
        self.assertIn('main "$@"', top_level_source)

    def test_every_changed_block_has_complete_current_or_before_unit_and_exact_diff(self) -> None:
        from generate_prompts import configured_semantic_units

        before = """echo old_bootstrap
...
"""
        temporary, root = self.make_tracked_shell(before)
        self.addCleanup(temporary.cleanup)
        (root / "sample.sh").write_text("""echo new_bootstrap

function demo() {
    echo new_body
}
""", encoding="utf-8")
        _, coverage = configured_semantic_units(root, self.synthetic_selection())
        self.assertTrue(coverage["all_changed_blocks_covered"])
        ...
        for block in coverage["changed_blocks"]:
            ...
            self.assertTrue(block["complete_current_units"] or block["complete_before_units"])

    def test_deletion_hunk_coordinates_are_zero_count_and_not_an_invented_line(self) -> None:
        from generate_prompts import _changed_blocks
...
"""
        temporary, root = self.make_tracked_shell(before)
        self.addCleanup(temporary.cleanup)
        (root / "sample.sh").write_text(before.replace("    echo delete_me\n", ""), encoding="utf-8")
    ...
    def test_representative_real_nested_group_function_is_complete(self) -> None:
        from generate_prompts import _shell_source_units
        ...
        parse_args = next(unit for unit in units if unit["unit_name"] == "parse_args")
        self.assertIn('require_optarg "${1}" "${2:-}" "main" || {', parse_args["source"])
        self.assertIn('fil_in="${2}"', parse_args["source"])
        self.assertTrue(parse_args["source"].rstrip().endswith("}"))

    def test_selection_loads_package_roles_scopes_questions_edges_and_units(self) -> None:
        root = Path(__file__).resolve().parents[3]
        selection = load_target_selection(
            SimpleNamespace(
                path_values=None,
        ...
        self.assertEqual(len(selection["evidence_selection"]["context_selectors"]), 5)
        self.assertEqual(
    ...
    def test_configured_units_cover_changed_blocks_and_complete_context_functions(self) -> None:
        from generate_prompts import configured_semantic_units
        ...
        wrap_units = [
            unit for unit in units if unit["path"] == "lib/bash/core/wrap_cmd.sh"
        ]
        self.assertIn("function get_submit_logs()", "\n".join(unit["source"] for unit in wrap_units))
        self.assertIn("function print_built_cmd()", "\n".join(unit["source"] for unit in wrap_units))
        for unit in wrap_units:
            if unit["unit_name"] in {"get_submit_logs", "print_built_cmd"}:
                self.assertTrue(unit["source"].rstrip().endswith("}"))
        context_units = [unit for unit in units if unit["evidence_role"] == "context"]
        self.assertEqual(len(context_units), 5)
        self.assertTrue(all(unit["unit_name"] == "run_jobs" for unit in context_units))
        self.assertTrue(all("get_submit_logs" in unit["source"] for unit in context_units))

    def test_configured_renderer_uses_identity_edges_questions_and_complete_units(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            markdown = (report / selection["package"]["semantic_review_path"]).read_text(
                encoding="utf-8"
            )

        self.assertEqual(record["bundle_id"], "download-fastqs-dependency-closure")
        self.assertIn(selection["package"]["statements"][0], markdown)
        self.assertIn("| Edge kind | Reachability | Runtime status |", markdown)
    ...
    def test_existing_reports_option_accepts_bounded_private_tmp_dry_render(self) -> None:
        from run import resolve_reports_base

        root = Path(__file__).resolve().parents[3]
        dry_base = Path("/private/tmp/dependency-closure-dry-base")
        self.assertEqual(resolve_reports_base(root, Path("/private/tmp")), Path("/private/tmp"))
        ...
        with self.assertRaisesRegex(ValueError, "artifacts/dev/audit or /private/tmp"):
            resolve_reports_base(root, root.parent / "unexpected-reports")

    def test_configured_rule_scopes_select_only_declared_rules_and_paths(self) -> None:
        from run import configured_rule_selection
        ...
        selection = {
            "package": {},
            "rule_scopes": [
                {"rule_id": "ONE", "paths": ["scripts/**/*.sh"], "classification": "strict"},
                {"rule_id": "TWO", "paths": ["tests/**/*.sh"], "classification": "advisory"},
            ],
        }
        selected, scopes = configured_rule_selection(rules, selection, None)
        self.assertEqual([rule["rule_id"] for rule in selected], ["ONE", "TWO"])
        self.assertEqual(scopes, {"ONE": ["scripts/**/*.sh"], "TWO": ["tests/**/*.sh"]})

    def test_bundle_rejects_every_incomplete_or_truncated_coverage_state(self) -> None:
        from generate_prompts import write_configured_bundle
        ...
        cases = {
            ...
            "uncovered hunk": {"uncovered_changed_blocks": [{"block_id": "one"}]},
        ...
        for label, mutation in cases.items():
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ValueError, "incomplete semantic-unit coverage"
                ),
            ):
                write_configured_bundle(
                    Path(tmp), self.minimal_bundle_selection(), [target], [], [], [],
                    [unit], coverage, "guard-test"
                )

    def test_dependency_graph_requires_declared_relationships_and_rejects_conflicts(self) -> None:
        from run import validate_dependency_graph

        root = Path(__file__).resolve().parents[3]
        raw = __import__("json").loads(
            (root / "dev/config/pilots/download_fastqs_dependency_closure.json").read_text(
                encoding="utf-8"
            )
        )
        known = {target["path"] for target in raw["targets"]} | {
            item["path"] if isinstance(item, dict) else item for item in raw["context"]
        }
        report = validate_dependency_graph(
            raw["dependency_edges"], known, raw["edge_requirements"]
        )
        self.assertEqual(
        ...
        missing = [edge for edge in raw["dependency_edges"] if (edge["from"], edge["to"]) != missing_identity]
        with self.assertRaisesRegex(ValueError, "missing required dependency edge"):
            validate_dependency_graph(missing, known, raw["edge_requirements"])

        duplicate = raw["dependency_edges"] + [copy.deepcopy(raw["dependency_edges"][0])]
        with self.assertRaisesRegex(ValueError, "duplicate dependency edge"):
            validate_dependency_graph(duplicate, known, raw["edge_requirements"])
        ...
        with self.assertRaisesRegex(ValueError, "contradictory dependency edge"):
            validate_dependency_graph(contradictory, known, raw["edge_requirements"])

        undeclared = copy.deepcopy(raw["dependency_edges"])
        undeclared[0]["to"] = "undeclared/path.sh"
        with self.assertRaisesRegex(ValueError, "undeclared dependency endpoint"):
            validate_dependency_graph(undeclared, known, raw["edge_requirements"])

    def test_actual_rule_scopes_report_exact_cohort_matches_and_enforcement(self) -> None:
        from run import configured_rule_scope_report
        ...
        self.assertEqual(report["SHELL.LINE_LENGTH"]["enforcement"], "advisory")

    def test_ordinary_function_modification_omits_redundant_complete_before_source(self) -> None:
        from generate_prompts import configured_semantic_units
    ...
    def test_whole_deleted_function_retains_complete_before_state_unit(self) -> None:
        from generate_prompts import configured_semantic_units
...
"""
        ...
        units, coverage = configured_semantic_units(root, self.synthetic_selection())
        removed = next(unit for unit in units if unit["unit_name"] == "removed")
    ...
    def test_partial_function_deletion_retains_exact_lines_without_full_before_unit(self) -> None:
        from generate_prompts import configured_semantic_units
...
"""
        ...
        self.assertTrue(all(block["represented"] for block in coverage["changed_blocks"]))

    def test_partial_top_level_deletion_uses_current_region_and_exact_diff(self) -> None:
        from generate_prompts import configured_semantic_units
        ...
        before_units = [unit for unit in units if unit["source_state"] == "before"]
        self.assertEqual(len(current), 1)
        self.assertEqual(before_units, [])
        self.assertIn("-echo deleted_line", "".join(current[0]["diff_evidence"]))
        self.assertTrue(coverage["all_changed_blocks_covered"])

    def test_coverage_metadata_fingerprints_without_duplicating_exact_diff(self) -> None:
        from generate_prompts import configured_semantic_units
        ...
        self.assertRegex(block["diff_evidence_fingerprint"], r"^sha256:[0-9a-f]{64}$")
        self.assertTrue(block["exact_diff_retained"])
    ...
    def test_markdown_contributions_cover_every_line_and_byte(self) -> None:
        ...
        root = Path(__file__).resolve().parents[3]
        selection = load_target_selection(
            SimpleNamespace(path_values=None, paths_from=Path("dev/config/pilots/download_fastqs_dependency_closure.json")),
            root,
        )
        units, coverage = configured_semantic_units(root, selection)
        targets = [
        ...
        with tempfile.TemporaryDirectory() as tmp:
            record = write_configured_bundle(
                Path(tmp), selection, targets, [], [], selection["adapter_limitations"],
                units, coverage, "contribution-test"
            )[0]
        ...
        self.assertEqual(sum(row["lines"] for row in contributions.values()), record["markdown_lines"])
        self.assertEqual(sum(row["bytes"] for row in contributions.values()), record["markdown_bytes"])

    def test_linked_children_filter_exact_ordered_partitions_and_rule_scopes(self) -> None:
        ...
        self.assertEqual(
            ...
            [item["path"] for item in runtime["linked_package"]["umbrella_target_ownership"]],
        )
        combined_edges = {
        ...
        self.assertEqual(
            [combined_edges[(edge["from"], edge["to"])] for edge in runtime["linked_package"]["umbrella_dependency_edges"]],
        ...
        for selection, patterns in (
            ...
            self.assertEqual(selection["rule_scope_report"]["SHELL.SYNTAX"]["matched_targets"], expected)
            self.assertEqual(
            ...
            self.assertEqual(selection["rule_scope_report"]["SHELL.LINE_LENGTH"]["matched_targets"], expected)

    def test_linked_configuration_rejects_child_ceiling_and_ownership_defects(self) -> None:
        def third_child(value: dict[str, object]) -> None:
            value["package"]["children"].append(copy.deepcopy(value["package"]["children"][0]))

        with self.assertRaisesRegex(ValueError, "at most two"):
            self.load_modified_linked_config(third_child)

        mutations = {
            "target": lambda value: value["targets"][0].update(subcohort="unowned"),
            "context": lambda value: value["context"][0].update(child="unknown-child"),
            "edge": lambda value: value["dependency_edges"][0].update(child="unknown-child"),
        }
        for label, mutate in mutations.items():
            with self.subTest(label=label), self.assertRaisesRegex(ValueError, "owner"):
                self.load_modified_linked_config(mutate)

        def duplicate_target_owner(value: dict[str, object]) -> None:
            value["package"]["children"][1]["subcohorts"].append("shared_runtime")

        with self.assertRaisesRegex(ValueError, "exactly one"):
            self.load_modified_linked_config(duplicate_target_owner)

    def test_linked_graph_validates_umbrella_endpoints_before_child_filtering(self) -> None:
        tests = self.load_child("download-fastqs-dependency-closure-tests-registration")
        cross_edges = [
            edge for edge in tests["dependency_edges"]
    ...
    def test_runtime_linked_semantic_units_preserve_final_counts(self) -> None:
        ...
        root = Path(__file__).resolve().parents[3]
        runtime = self.load_child("download-fastqs-dependency-closure-runtime-production")
        runtime_units, runtime_coverage = configured_semantic_units(root, runtime)
        self.assertEqual(
    ...
    def test_linked_children_share_bounded_final_size_limits(self) -> None:
        for child in (
            ...
            with self.subTest(child=child):
                ...
                self.assertEqual(limits["semantic_markdown_max_bytes"], 1048576)
    ...
    def test_linked_renderer_names_sibling_and_shared_graph(self) -> None:
        ...
        root = Path(__file__).resolve().parents[3]
        selection = self.load_child("download-fastqs-dependency-closure-runtime-production")
        units, coverage = configured_semantic_units(root, selection)
        targets = [
            {
                **target,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            write_configured_bundle(
                Path(temporary), selection, targets, [], [], selection["adapter_limitations"],
                units, coverage, "linked-render-test",
            )
            markdown = (Path(temporary) / selection["package"]["semantic_review_path"]).read_text(encoding="utf-8")
        self.assertIn("not the entire closure", markdown)
        self.assertIn(selection["linked_package"]["sibling_package_id"], markdown)
        self.assertIn(selection["linked_package"]["graph_ownership_fingerprint"], markdown)
        production_edges = (
            markdown.count("→ `bin/")
            + markdown.count("→ `lib/bash/")
        )
        self.assertEqual(production_edges, 7)

    def test_linked_pair_header_validation_rejects_identity_and_fingerprint_mismatches(self) -> None:
        from run import validate_linked_pair_headers

        runtime = self.load_child("download-fastqs-dependency-closure-runtime-production")["linked_package"]
        tests = self.load_child("download-fastqs-dependency-closure-tests-registration")["linked_package"]
        pilots = [
            {
        ...
        cases = {
            ...
            "graph fingerprint": (1, "graph_ownership_fingerprint", "sha256:wrong"),
        }
    ...
    def test_linked_partition_union_rejects_gaps_overlaps_and_contradictions(self) -> None:
        from run import validate_linked_partition_union

        runtime = self.load_child("download-fastqs-dependency-closure-runtime-production")
        tests = self.load_child("download-fastqs-dependency-closure-tests-registration")
        linkage = runtime["linked_package"]
        targets = [item["path"] for item in runtime["targets"] + tests["targets"]]
        contexts = list(runtime["context"] + tests["context"])
        edges = list(runtime["dependency_edges"] + tests["dependency_edges"])
        validate_linked_partition_union(linkage, targets, contexts, edges, ["hunk-a", "hunk-b"])
        cases = {
            "target gap": (targets[:-1], contexts, edges, ["hunk-a"]),
            "target overlap": ([*targets[:-1], targets[0]], contexts, edges, ["hunk-a"]),
            "context gap": (targets, contexts[:-1], edges, ["hunk-a"]),
            "edge gap": (targets, contexts, edges[:-1], ["hunk-a"]),
            "edge overlap": (targets, contexts, [*edges, copy.deepcopy(edges[0])], ["hunk-a"]),
            "edge contradiction": (targets, contexts, [{**edges[0], "runtime_status": "exercised"}, *edges[1:]], ["hunk-a"]),
