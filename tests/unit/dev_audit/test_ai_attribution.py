...
class AiAttributionTest(unittest.TestCase):
    ...
    def test_model_declaration_rejects_residue_and_bad_separators(
        self,
    ) -> None:
        invalid = (
            "banana, GPT-5.6, ???",
        ...
        for models in invalid:
            header = f"# OpenAI Codex ({models}) was used in development."
    ...
    def test_malformed_attribution_like_header_fails_standing_scan(
        self,
    ) -> None:
        ...
        self.assertIn(
            ...
            {
                item.rule_id
                for item in check_attribution_source(text, "tool.sh")
            },
        )

    def test_single_and_combined_tool_forms_are_truthful_and_accepted(
        self,
    ) -> None:
        headers = (
    ...
    def test_exact_model_counters_do_not_confuse_gpt5_and_gpt56(self) -> None:
        ...
        self.assertTrue(any("GPT-5'" in item.message for item in findings))
        self.assertFalse(
    ...
    def test_generic_series_tokens_are_parsed_without_substrings(self) -> None:
        ...
        self.assertTrue(
            any(
                item.rule_id == "SOURCE.HEADER.AI_ATTRIBUTION"
                for item in findings
            ),
        )

    def test_repeatable_and_future_required_models_are_exact_data(
        self,
    ) -> None:
        text = source_file(
            (
    ...
    def test_missing_attribution_requires_domain_models_and_tools(
        self,
    ) -> None:
    ...
    def test_body_attribution_does_not_change_no_ai_header_profile(
        self,
    ) -> None:
        text = source_file(None) + (
            "printf 'body\\n'\n"
    ...
    def test_boundary_profiles_and_body_spacing(self) -> None:
        ...
        self.assertIn(
            ...
            {
                item.rule_id
                for item in check_attribution_source(wrong, "tool.py")
            },
        )
        ...
        self.assertIn(
            ...
            {
                item.rule_id
                for item in check_attribution_source(one_blank, "tool.py")
            },
        )
    ...
    def test_manifest_and_inventory_compare_observed_requirements(
        self,
    ) -> None:
        ...
        self.assertEqual(
            rows[0]["observed_contribution_domain"],
            "development",
        )
        self.assertEqual(
            rows[0]["required_contribution_domain"],
            "development",
        )
