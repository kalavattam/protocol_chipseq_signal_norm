...
class SourcePolicyTest(unittest.TestCase):
    ...
    def test_static_source_requires_exact_repository_relative_mapping(self) -> None:
    ...
    def test_semantic_test_path_maps_shared_test_helpers(self) -> None:
        record = classify_source(
            ...
            'source "$(git rev-parse --show-toplevel)/tests/support/test_helpers.sh"',
        )
        self.assertEqual(record.classification, "static")
    ...
    def test_fixture_recipe_maps_shared_fixture_helpers(self) -> None:
        record = classify_source(
            ...
            'source "$(git rev-parse --show-toplevel)/tests/support/fixture_helpers.sh"',
        )
        self.assertEqual(record.classification, "static")
    ...
    def test_stale_sc1090_for_static_source_is_reported(self) -> None:
        findings = suppression_mismatches(
            ...
            'source "${_dir_scr_help}/functions/help/help_submit_align_fastqs.sh"\n',
        )
    ...
    def test_bootstrap_help_source_requires_one_blank_before_directive(self) -> None:
        text = (
            '_dir_scr_help="$(pwd)"\n'
        ...
        findings = bootstrap_help_spacing_warnings(
            "bin/submit_align_fastqs.sh", text
        )
    ...
    def test_bootstrap_help_source_with_one_blank_passes(self) -> None:
        ...
        self.assertEqual(
            bootstrap_help_spacing_warnings("bin/submit_align_fastqs.sh", text),
            [],
        )
    ...
    def test_submit_bootstrap_repository_policy_is_green(self) -> None:
        ...
        self.assertTrue(
            all(row.current_shebang == CANONICAL_BASH_SHEBANG for row in inventory)
        )
