...
# -*- coding: utf-8 -*-
#
# Script: test_shell_help_pilot.py
#
...
from dev.audit.parse_findings import CommandResult, parse_result
from dev.audit.run import (
...
from dev.audit.shell_help_pilot import (
...
class ShellHelpPilotTest(unittest.TestCase):
    ...
    def test_target_states_identify_untracked_and_staged_worktree(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = self.make_repo(Path(temp_dir) / "repo")
            ...
            status = {
                line[3:]: {"path": line[3:], "git_status": line[:2]}
            ...
            self.assertEqual(
                selected_git_state(root, "new.txt", status)[
                    "git_state_labels"
                ],
                ["untracked"],
            )

            labels = selected_git_state(root, "tracked.txt", status)[
                "git_state_labels"
            ]
    ...
    def test_untracked_primary_is_explicit_new_file_evidence_and_stale(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            target = {
                ...
                "path": "new.sh",
                "role": "primary",
                "git_state_labels": ["untracked"],
                "content_fingerprint": "sha256:" + "a" * 64,
            }

            records = write_pilot_bundle(
                report,
                [target],
                [],
                [],
    ...
    def test_help_module_is_not_misclassified_as_a_top_level_wrapper(
        self,
    ) -> None:
    ...
    def test_help_body_accepts_owned_comments_before_heredoc(self) -> None:
        from dev.audit.shell_help_pilot import help_body
...
"""
        ...
        self.assertEqual(
            {item["pattern"] for item in facts[0]["value"]},
            {"-ds|--dir[_-]scr", "-h|--help", "-*"},
        )
    ...
    def alias_comparison(
        ...
        """
        Return resolved facts and findings for one interface.
        """
    ...
    def test_long_only_help_row_omits_public_short_alias(self) -> None:
        ...
        documented = next(
            item for item in facts if item["topic"] == "documented_aliases"
        )

        self.assertEqual(documented["value"], ["--queue_name"])
        self.assertEqual([item["evidence"] for item in findings], ["-q"])

    def test_help_metavariable_does_not_prevent_exact_alias_matching(
        self,
    ) -> None:
        _, findings = self.alias_comparison(
            (
    ...
    def test_externalized_help_is_bound_to_owning_wrapper(self) -> None:
        ...
        association = next(
            item
            for item in facts
            if item["topic"] == "documentation_source_associations"
        )

        self.assertEqual(
            association["value"]["documentation_source"],
            "docs/tool_alpha_help.sh",
        )
        self.assertEqual(findings, [])

    def test_hidden_systematic_hyphen_alias_is_not_required_in_help(
        self,
    ) -> None:
        facts, findings = self.alias_comparison(
            (
        ...
        self.assertEqual(
            resolved["--queue-name"]["visibility"],
            "hidden_systematic_compatibility",
        )
        self.assertEqual(findings, [])
    ...
    def test_duplicate_documented_alias_is_reported(self) -> None:
        ...
        self.assertTrue(
            any("duplicate" in str(item["message"]) for item in findings),
        )
    ...
    def test_removed_aliases_are_recorded_as_rejected_policy(self) -> None:
        ...
        removed = next(
            item
            for item in facts
            if item["topic"] == "removed_alias_classifications"
        )
        ...
        self.assertTrue(
            {row["alias"] for row in removed["value"]}.isdisjoint(resolved),
        )
        self.assertEqual(findings, [])
    ...
    def test_download_pilot_has_no_help_documentation_findings(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[3]
        config = json.loads(
            (
                root / "dev/config/pilots/download_fastqs_shell_help.json"
            ).read_text(encoding="utf-8"),
        )

        interface_paths = {item["path"] for item in config["interfaces"]}
        source_paths = {
            item["documentation_source"] for item in config["interfaces"]
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            proc = subprocess.run(
                [
                    sys.executable,
                    str(AUDIT_DIR / "shell_help_pilot.py"),
                    "--mode",
                    "interface_facts",
                    "--snapshot",
                    str(snapshot_path),
        ...
        alias_findings = [
            item for item in payload["findings"] if item["topic"] == "aliases"
        ]
        ...
        classifications = {
            row["path"]: {
                item["alias"]: item["acceptance"] for item in row["value"]
            }
        ...
        self.assertEqual(
            classifications["bin/execute_download_fastqs.sh"],
            {"-i": "rejected", "--infile": "rejected", "-eo": "rejected"},
        )
...
"""
    ...
    def test_bundle_fences_source_comments_and_has_human_sections(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
            ...
            facts = [
                {
                    "topic": "line_length_candidates",
                    "path": "new.sh",
                    "value": [
                        {
                            "line": 1,
                            "length": 99,
                            "excerpt": "1: # not a heading",
                            "exception_cues": [],
            ...
            records = write_pilot_bundle(
                ...
                [],
                {},
                lambda _: "# not a heading\necho hi\n",
                80,
                {"aliases": "## Rule heading\n# comment"},
                {},
            )
            body = (report / records[0]["path"]).read_text(encoding="utf-8")
    ...
    def test_wrapper_examples_and_registry_runtime_case_are_findings(
        self,
    ) -> None:
        text = """
function help_submit_download_fastqs() {
...
"""
        ...
        self.assertTrue(
            any("two materially" in item["message"] for item in findings),
        )
        ...
        command_reference_facts(
            ...
            callables,
            concepts,
            {"runtime_version", "external_programs"},
            facts,
            findings,
            questions,
        )
        ...
        self.assertTrue(
            any(
                item.get("topic") == "runtime_requirements"
...
"""
        ...
        self.assertEqual(
            candidate["source_checker_relationship"],
            "excluded_by_documented_source_checker_policy",
        )

    def test_verify_marks_an_untracked_target_stale(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            public = self.make_repo(Path(temp_dir) / "public")
            private = self.make_repo(Path(temp_dir) / "private")

            (public / "new.txt").write_text("before\n", encoding="utf-8")
            (public / "baseline.json").write_text("{}\n", encoding="utf-8")
            (public / "rules.toml").write_text(
                "schema_version = 1\n",
                encoding="utf-8",
            )

            fingerprint, _ = entry_fingerprint(public, "new.txt")
            baseline_fingerprint, _ = entry_fingerprint(
                public,
                "baseline.json",
            )
            rules_fingerprint, _ = entry_fingerprint(
                public,
                "rules.toml",
            )
            ...
            (report / "targets.ndjson").write_text(
                json.dumps(
                    {
                        "schema_version": 1,
                        "run_id": "test",
            ...
            (report / "semantic_review.ndjson").write_text(
                json.dumps(
                    {
                        "schema_version": 1,
                        "run_id": "test",
                        "bundle_id": "test",
                        "path": "semantic_review/test.md",
                        "target_fingerprints": {"new.txt": fingerprint},
                    },
                )
                + "\n",
                encoding="utf-8",
            )
            (report / "pilot_report.json").write_text(
    ...
    def test_renderer_uses_artifact_findings_and_omits_resolved_decisions(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
            ...
            findings = [
                {
                    ...
                    "path": target["path"],
                    "message": "noncanonical placeholder",
                    "line": 1,
                },
            ]
            questions = [
                {
                    "topic": "other",
                    "question": "Resolve a remaining question.",
                    "paths": [target["path"]],
                },
            ]

            record = write_pilot_bundle(
                report,
                [target],
                [],
                [],
            ...
            self.assertIn(
                "1 deterministic findings:\n- 1 noncanonical placeholder:",
                body,
            )
            self.assertIn("Resolve a remaining question.", body)
            self.assertNotIn(
                "Resolve runtime-requirements capitalization policy",
                body,
            )
    ...
    def test_injected_findings_change_each_rendered_topic_result(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            topics = (
                ...
                "aliases",
                "examples",
                "types_and_placeholders",
                "requiredness_defaults",
    ...
    def test_alias_comparison_keeps_fallback_and_compatibility_separate(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
                    ...
                    "value": [
                        {
                            ...
                            "locations": [
                                {"function": "parse_args", "line": 2},
                            ],
                        },
                        {
                            "alias": "-ds",
                            "visibility": "hidden_short_compatibility",
                            "retention": "indefinite",
                            "locations": [
                                {"function": "parse_args", "line": 1},
                            ],
                        },
                        {
                            "alias": "--verbose",
                        ...
                        },
                        {
                            ...
                            "retention": "indefinite",
                            "locations": [{"function": "main", "line": 1}],
                        },
                    ],
                },
                {
                    "topic": "documented_aliases",
                    "path": "lib/bash/help/help_submit_download_fastqs.sh",
                    "value": ["-ds"],
                },
            ]
    ...
    def test_alias_results_retain_public_and_hidden_aliases_without_retirement(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
    ...
    def test_untracked_help_evidence_contains_complete_help_body(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            target = {
                "schema_version": 1,
                "path": "lib/bash/help/help_submit_x.sh",
                "role": "primary",
                "git_state_labels": ["untracked"],
    ...
    def test_runtime_topic_renders_exact_callable_rule(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            rule = (
                "[HELP.RUNTIME.EXACT_CALLABLE]\nUse exact callable spellings "
                "such as `bash`, `bamCompare`, and `multiBigwigSummary`; "
                "conceptual prose may use project names."
            )
    ...
    def test_registry_preserves_mixed_case_and_rejects_duplicate_callables(
        self,
    ) -> None:
        registry = {
            "schema_version": 1,
            "commands": [
                ...
                {
                    "callable": "multiBigwigSummary",
                    "conceptual_names": ["deepTools"],
                },
            ],
        }
        ...
        self.assertEqual(
            concepts["deepTools"],
            {"bamCompare", "multiBigwigSummary"},
        )

        duplicate = copy.deepcopy(registry)
        duplicate["commands"].append(
            {"callable": "bamCompare", "conceptual_names": []},
        )

        with self.assertRaisesRegex(
            ValueError,
            "duplicate command registry callable",
        ):
            validate_command_registry(duplicate)

    def test_public_registry_and_rule_have_current_bounded_consumption(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[3]
        registry = json.loads(
        ...
        config = tomllib.loads(
            (root / "dev/config/rules.toml").read_text(encoding="utf-8"),
        )
    ...
    def test_generic_resolution_rejects_unknown_or_ambiguous_names(
        self,
    ) -> None:
        callables, concepts = validate_command_registry(
            {
...
"""
        ...
        ambiguous_question = "ambiguous runtime-command reference 'deepTools'"
        unknown_question = "unknown runtime-command reference 'mysteryTool'"

        self.assertTrue(
            any(
                "'bash' instead of conceptual name 'Bash'" in item["message"]
                for item in findings
            ),
        )
        self.assertFalse(
            any("bamCompare" in item["message"] for item in findings),
        )
        self.assertTrue(
            any(ambiguous_question in item["question"] for item in questions),
        )
        self.assertTrue(
            any(unknown_question in item["question"] for item in questions),
        )
    ...
    def test_command_reference_adapter_consumes_snapshot_registry(
        self,
    ) -> None:
        registry = {
            "schema_version": 1,
            "commands": [
                {
                    "callable": "bamCompare",
                    "conceptual_names": ["deepTools bamCompare"],
                },
            ],
        }
        source = """
...
"""
        snapshot = {
            ...
            "context": [
                {"path": "registry.json", "content": json.dumps(registry)},
            ],
            "adapter_config": {
                "registry_path": "registry.json",
                "reference_scopes": ["external_programs"],
            },
        }
        ...
        self.assertEqual(
            ...
            (
                ...
                "'bamCompare' instead of conceptual name 'deepTools "
                "bamCompare'"
            ),
        )
    ...
    def test_bundle_validates_all_rendered_tables_and_groups_line_candidates(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
                    ...
                    "value": [
                        {
                            ...
                            "excerpt": '9: x\n10: log_out_a="long"\n11: x',
                            "exception_cues": [],
                            "location_kind": "ordinary_shell_code",
                            "source_checker_relationship": (
                                "independent_pilot_only_candidate"
                            ),
                            "classification_status": "semantic/manual",
                        },
                        {
            ...
            self.assertEqual(
                record["line_length_rendering"]["detailed_candidate_count"],
                2,
            )
            self.assertEqual(
                record["line_length_rendering"]["grouped_candidate_count"],
                1,
            )
            self.assertIn(
                "Not applicable; this topic is semantic-review only.",
    ...
    def test_supporting_gap_requires_a_real_submit_command_boundary(
        self,
    ) -> None:
        quoted = (
            'TEST_NAME="x"\nassert_pattern_found cfg "${TEST_BASH} '
            '${ROOT_REPO}/bin/submit_download_fastqs.sh"\nbash '
            "bin/execute_download_fastqs.sh --fil_in x --dir_out y --dir_sym "
            "z\nthen\n"
        )
        facts: list[dict[str, object]] = []
        supporting_alignment(
        ...
        self.assertEqual(
            facts[0]["value"]["apparent_gap_or_uncertainty"],
            "No direct submit-wrapper assertion.",
        )

        direct = (
            'TEST_NAME="x"\nrun_capture x log "${TEST_BASH}" '
            '"${ROOT_REPO}/bin/submit_download_fastqs.sh" arg\nbash '
            "bin/execute_download_fastqs.sh --fil_in x --dir_out y --dir_sym "
            "z\nthen\n"
        )
        facts = []
    ...
    def test_focused_diff_uses_complete_line_windows_for_oversized_hunks(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = self.make_repo(Path(temp_dir) / "repo")
            ...
            subprocess.run(
                ["git", "-C", str(root), "commit", "-qm", "large baseline"],
                check=True,
            )
    ...
    def test_focused_diff_reports_when_no_complete_line_window_fits(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = self.make_repo(Path(temp_dir) / "repo")
    ...
    def test_execute_windows_require_environment_and_lifecycle_source(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[3]
        target_content = {
        ...
        self.assertIn(
            'handle_env "${env_nam}"',
            windows["environment_resolution"]["rendered_evidence"],
        )
        self.assertIn(
            "setup_env               || return 1",
            windows["final_dispatch"]["rendered_evidence"],
        )
        self.assertIn(
            "run_jobs                || return 1",
            windows["final_dispatch"]["rendered_evidence"],
    ...
    def test_submit_and_environment_anchor_windows_render_named_behavior(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[3]
        target_content = {
        ...
        self.assertIn(
            "-h|--hlp|--help)",
            submit["early_help"]["rendered_evidence"],
        )
        self.assertIn(
        ...
        for call in (
            ...
            self.assertIn(
                call,
                submit["worker_after_bootstrap"]["rendered_evidence"],
            )

        environment = windows["lib/bash/help/help_execute_download_fastqs.sh"][
        ...
        self.assertEqual(
            environment["evidence_kind"],
            "complete_line_source_window",
        )

    def test_anchor_validation_rejects_deleted_and_descriptive_substitutes(
        self,
    ) -> None:
        root = Path(__file__).resolve().parents[3]
        target_content = {
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(
                ...
                write_pilot_bundle(
                    ...
                    [],
                    {},
                    target_content.__getitem__,
                    12000,
                    anchor_evidence={submit_path: deleted_only},
                )
            ...
            with self.assertRaisesRegex(
                ...
                write_pilot_bundle(
                    ...
                    target_content.__getitem__,
                    12000,
                    anchor_evidence={submit_path: descriptive_only},
                )
            ...
            with self.assertRaisesRegex(
                ...
                write_pilot_bundle(
                    ...
                    target_content.__getitem__,
                    12000,
                    anchor_evidence={help_path: insufficient_environment},
                )
            ...
            with self.assertRaisesRegex(
                ValueError,
                "missing required behavioral anchor group argument_parsing",
            ):
                write_pilot_bundle(
                    report,
                    [target(execute_path)],
                    ...
                    target_content.__getitem__,
                    12000,
                    anchor_evidence={execute_path: deleted_parser},
                )
    ...
    def test_anchor_metadata_uses_its_exact_rendered_evidence(self) -> None:
        ...
        targets = [
            {
                ...
                "role": "primary",
                "git_state_labels": ["untracked"],
                "content_fingerprint": "sha256:" + str(index) * 64,
                "checks_run": [],
                "findings_count": 0,
            }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected = evidence["bin/execute_download_fastqs.sh"][
                "environment_resolution"
            ]["rendered_evidence"]
            metadata = next(
                item
                for item in record["behavioral_anchor_coverage"]
                if item["path"] == "bin/execute_download_fastqs.sh"
                and item["anchor_group"] == "environment_resolution"
            )

            self.assertEqual(metadata["rendered_evidence"], expected)
            self.assertNotEqual(
                metadata["rendered_evidence"],
                focused_diff_excerpt(
                    root,
                    "bin/execute_download_fastqs.sh",
                    12000,
    ...
    def test_missing_execute_anchor_windows_fail_bundle_generation(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report = Path(temp_dir) / "report"
            ...
            with self.assertRaisesRegex(
                ...
                write_pilot_bundle(
                    ...
                    lambda _: "echo x\n",
                    80,
                    anchor_evidence={target["path"]: {}},
                )

    def test_environment_provision_fences_are_balanced_after_bounding(
        self,
    ) -> None:
        text = """
...
"""
        provision = bounded_markdown_provision(
            text,
            "Recommended environment:",
            4000,
        )
        ...
        bounded = bounded_markdown_provision(
            text,
            "Recommended environment:",
            len(
                (
                    b"### Recommended environment:\n"
                    b"Use the project environment.\n\n"
                ),
            ),
        )
    ...
    def test_pilot_report_self_hash_excludes_its_own_hash_field(self) -> None:
        report = {
            ...
            "run_id": "run",
            "package_provenance": {
                "artifact_hashes": {"facts.ndjson": "sha256:facts"},
                "pilot_report_self_hash": {
                    "algorithm": "sha256",
                    "canonicalization": (
        ...
        report["package_provenance"]["pilot_report_self_hash"]["value"] = (
            digest
        )

        self.assertEqual(pilot_report_self_hash(report), digest)

    def controlled_smoke_selection(
        self,
        ...
        args = SimpleNamespace(
            path_values=None,
            paths_from=Path(
                "dev/config/pilots/download_fastqs_shell_help.json",
            ),
        )
    ...
    def controlled_smoke_capture(
        ...
        **overrides: object,
        ...
        messages = [
            message
            for group in evidence["required_pass_groups"].values()
            for message in group
        ]

        if include_observations:
            messages.extend(
                observation["pass_message"]
                for observation in evidence.get(
            ...
            "stdout": "\n".join(
                [
                    *(f"PASS: {message}" for message in messages),
                    (
                        "Summary for submit download-fastqs local: pass=40 "
        ...
        capture = {
            "schema_version": 1,
            "run_id": "runtime-test",
            "manifest_path": selection["selection_path"],
            ...
            "outer_context": {"project_environment_active": False},
            "outer_context_ok": True,
            "source_fingerprints_before": controlled_smoke_sources(
                root,
                target,
            ),
            "source_fingerprints_after": controlled_smoke_sources(
                root,
                target,
            ),
            "raw": raw,
    ...
    def test_declarative_controlled_smoke_source_coverage_and_runtime_evidence(
        self,
    ) -> None:
        ...
        self.assertTrue(
            all(
                group["present"]
                for group in coverage["coverage_groups"].values()
            ),
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            capture_path.write_text(
                json.dumps(
                    self.controlled_smoke_capture(root, selection, target),
                ),
                encoding="utf-8",
            )
            args = SimpleNamespace(
                run_id="runtime-test",
                normalize_controlled_smoke=capture_path,
                runtime_evidence_out=evidence_path,
            )

            self.assertEqual(
                normalize_controlled_smoke(args, root, selection),
                0,
            )
            ...
            self.assertEqual(
                normalized["environment_selection"],
            ...
            self.assertEqual(
                normalized["worker_execution"]["status"],
                "passed",
            )
            self.assertEqual(
                normalized["worker_execution"]["environment_evidence"],
                "not_directly_evidenced",
            )
            self.assertEqual(
                normalized["observations"],
                {
                    name: {
                        ...
                        "evidence": {
                            "kind": "pass_message",
                            "message": item["pass_message"],
                        },
                    }
                    for name, item in target["evidence"][
                        "optional_observations"
                    ].items()
                },
            )
    ...
    def test_generator_environment_is_not_worker_environment_evidence(
        self,
    ) -> None:
        root, selection, target = self.controlled_smoke_selection()
        capture = self.controlled_smoke_capture(root, selection, target)

        capture["outer_context"] = {
            "conda_default_env": "env_protocol",
            "project_environment_active": True,
        }
    ...
    def test_optional_observations_are_not_required_for_valid_core_evidence(
        self,
    ) -> None:
        root, selection, target = self.controlled_smoke_selection()
    ...
    def test_generic_controlled_smoke_schema_does_not_require_observations(
        self,
    ) -> None:
        ...
        self.assertEqual(
            validate_controlled_smoke_evidence(evidence),
            evidence,
        )
    ...
    def test_controlled_smoke_core_evidence_fails_closed(self) -> None:
        ...
        skipped_raw = {
            "stdout": (
                "Summary for submit download-fastqs local: pass=1 fail=0 "
                "warn=0 skip=1"
            ),
            "stderr": "",
        }
        ...
        source_mismatched = self.controlled_smoke_capture(
            root,
            selection,
            target,
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for name, capture in cases:
                capture_path = Path(temp_dir) / f"capture-{name}.json"
                evidence_path = Path(temp_dir) / f"evidence-{name}.json"
                capture_path.write_text(json.dumps(capture), encoding="utf-8")
                args = SimpleNamespace(
                    run_id="runtime-test",
                    normalize_controlled_smoke=capture_path,
    ...
    def test_runtime_fact_verification_remains_optional_for_historical_reports(
        self,
    ) -> None:
        self.assertEqual(
            verify_runtime_fact_rows([], Path(__file__).resolve().parents[3]),
            [],
        )
    ...
    def test_five_file_supplied_package_contract_is_unchanged(self) -> None:
        ...
        values = [
            ...
            and any(
                isinstance(target, ast.Name) and target.id == "supplied"
