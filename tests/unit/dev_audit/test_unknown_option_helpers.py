...
from unknown_option_helpers import check_text, scan_repository

FUNCTION_HELP = """function demo() {{
    local show_help
    show_help=$(cat << EOM
Usage
-----
...
class UnknownOptionHelpersTest(unittest.TestCase):
    ...
    def test_help_emission_and_return_are_required(self) -> None:
        missing_help = FUNCTION_HELP.replace('            echo "${{show_help}}" >&2\n', '')
        ...
        self.assertTrue(
            check_text("lib/bash/core/demo.sh", missing_help.format(error=error))
        )
        self.assertTrue(
            check_text("lib/bash/core/demo.sh", missing_status.format(error=error))
        )
