...
from help_heredoc_reflow import Heredoc
from help_runtime_requirements import (
...
from help_style import exact_callable_message
...
def document(body: str) -> Heredoc:
    return Heredoc("demo", "EOM", 1, len(body.splitlines()) + 1, tuple(enumerate(body.splitlines(), 1)))
...
class RuntimeRequirementsTest(unittest.TestCase):
    def policy_rule_ids(self, *entries: str) -> set[str]:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = Path(tmp) / "runtime_policy.sh"
            source.write_text("function demo() {\n    :\n}\n", encoding="utf-8")
            body = (
    ...
    def test_singleton_is_unbulleted(self) -> None:
        label, entries, grammar = runtime_entries(document("Notes\n-----\n  Runtime requirements:\n    bash >= 4.4"))
    ...
    def test_categories_are_rejected(self) -> None:
        _, _, grammar = runtime_entries(document("Notes\n-----\n  Runtime requirements:\n    - Programs\n      + bash >= 4.4"))
    ...
    def test_semantic_candidate_signature_is_stable(self) -> None:
        evidence = Evidence("demo", "demo.sh", 4, "unknown_callable", "tool", "command -v tool", "direct", "conditional", "available in PATH", True, "low")
        self.assertEqual(candidate("demo", evidence, ["bash >= 4.4"]), candidate("demo", evidence, ["bash >= 4.4"]))

    def test_executable_requirements_use_exact_cli_casing(self) -> None:
        callables = {"bowtie2", "bwa", "bwa-mem2", "python", "Rscript", "samtools"}
        concepts = {
            "Bowtie2": {"bowtie2"},
            "BWA": {"bwa"},
            "BWA-MEM2": {"bwa-mem2"},
        ...
        self.assertIsNone(exact_callable_message("bowtie2", callables, concepts))
        self.assertIsNone(exact_callable_message("bwa", callables, concepts))
        self.assertIsNone(exact_callable_message("bwa-mem2", callables, concepts))
        self.assertIsNone(exact_callable_message("python", callables, concepts))
        self.assertIsNone(exact_callable_message("Rscript", callables, concepts))
        self.assertIsNone(exact_callable_message("samtools", callables, concepts))
    ...
    def test_python_runtime_floor_accepts_exact_executable_spellings(self) -> None:
        for requirement in (
            "python >= 3.11",
            "python3 >= 3.11",
    ...
    def test_runtime_conditions_accept_straight_single_quoted_api_syntax(self) -> None:
        for requirement in (
    ...
    def test_runtime_conditions_reject_unquoted_or_malformed_api_syntax(self) -> None:
        for requirement in (
            "parallel (when --slurm is not specified)",
            "parallel (when `--slurm` is not specified)",
            "bowtie2 (when '--aligner' bowtie2 is specified)",
    ...
    def test_sentence_style_resource_capitalization_accepts_prose(self) -> None:
        for requirement in (
    ...
    def test_sentence_style_resource_capitalization_rejects_lowercase_prose(self) -> None:
        for requirement in (
            "a compatible Conda environment providing the listed tools",
            "caller-supplied command executable",
            "reference FASTA and required index",
    ...
    def test_lowercase_executable_names_are_not_sentence_capitalized(self) -> None:
        for requirement in ("samtools", "bowtie2", "Rscript"):
            rules = self.policy_rule_ids("bash >= 4.4", requirement)
            self.assertNotIn("RUNTIME.CAPITALIZATION", rules)

    def test_conda_environment_terminology_accepts_resource_and_activation(self) -> None:
        for requirement in (
    ...
    def test_conda_environment_terminology_rejects_mamba_or_lowercase_resource(self) -> None:
        for requirement in (
    ...
    def test_runtime_sort_key_uses_exact_text_as_casefold_tie_breaker(self) -> None:
        from help_runtime_requirements import runtime_sort_key

        values = ["alpha resource", "Alpha resource"]
        self.assertEqual(sorted(values, key=runtime_sort_key), ["Alpha resource", "alpha resource"])

    def test_runtime_normalizer_preserves_case_conditions_and_singletons(self) -> None:
        from help_runtime_requirements import normalize_runtime_entries
    ...
    def test_resource_prose_is_not_treated_as_an_executable(self) -> None:
        ...
        self.assertIsNone(exact_callable_message(resource, callables, concepts))
    ...
    def test_settled_owner_requirements_distinguish_execution_from_validation(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/core/check_env.sh::check_pgrm_path"),
            ("bash >= 4.4",),
        )
        self.assertEqual(
            settled_requirements("lib/bash/core/construct_find.sh::construct_find"),
            ("bash >= 4.4",),
        )
        self.assertEqual(
    ...
    def test_settled_owner_requirements_preserve_runtime_alternatives_and_conditions(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/dispatch/manage_parallel.sh::determine_cores"),
            ("bash >= 4.4", "nproc, sysctl, or getconf"),
        )

    def test_c4_alignment_contract_has_conditional_aligner_providers_without_versions(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/workflows/align_fastqs.sh::align_fastqs"),
            (
                "bash >= 4.4",
                "bowtie2 (when '--aligner bowtie2' is specified)",
    ...
    def test_c4_filter_contracts_couple_cram_resources_and_keep_delegated_tools_conditional(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/workflows/filter_alignment.sh::_validate_args_filter_alignment"),
            ("bash >= 4.4", "dirname"),
        )
        self.assertEqual(
            settled_requirements("lib/bash/workflows/filter_alignment.sh::_check_chr_alignment"),
            (
                "bash >= 4.4",
                "awk",
        ...
        self.assertEqual(
            settled_requirements("lib/bash/workflows/filter_alignment.sh::_finalize_alignment_filter"),
            (
                "bash >= 4.4",
        ...
        self.assertIn(
            ...
            settled_requirements("lib/bash/workflows/filter_alignment.sh::filter_alignment_sc"),
        )
        self.assertNotIn(
            "Input BAM or CRAM index",
            settled_requirements("lib/bash/workflows/filter_alignment.sh::filter_alignment_sp"),
        )

    def test_c4_bam_cram_contracts_distinguish_validation_execution_and_resources(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_alignments.sh::_validate_process_alignments"),
            ("bash >= 4.4", "dirname"),
        )
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_alignments.sh::qsort_file_alignments"),
            (
                "bash >= 4.4",
                "Reference FASTA and required index (when processing CRAM)",
        ...
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_alignments.sh::convert_alignments_bed_python"),
            (
                "bash >= 4.4",
                "python >= 3.11",
                "Reference FASTA and required index (when processing CRAM)",
    ...
    def test_c4_zcat_is_one_conditional_alternative_provider_contract(self) -> None:
        self.assertEqual(
            DECOMPRESSION_REQUIREMENT,
            "gzip or zcat (when processing a gzipped bedGraph)",
        )
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_region.sh::check_region_bdg"),
            (
                "bash >= 4.4",
        ...
        alternatives = [item for item in evidence if item.key == "gzip_or_zcat"]
        self.assertEqual([item.invocation for item in alternatives], ["command -v gzip", "command -v zcat"])
        self.assertTrue(all(item.category == "callable_alternative" for item in alternatives))

    def test_c4_generated_atria_command_is_executed_and_internal_python_path_is_excluded(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_sequences.sh::trim_fastqs_atria"),
    ...
    def test_c4_table_contracts_preserve_direct_and_delegated_parser_tools(self) -> None:
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_tables.sh::_parse_table_core"),
            (
                "bash >= 4.4",
                "awk",
        ...
        self.assertEqual(
            settled_requirements("lib/bash/workflows/process_tables.sh::parse_table_simple"),
            ("bash >= 4.4", "awk", "gawk"),
        )

    def test_c4_settled_conditions_use_exact_grammar_and_no_tool_versions(self) -> None:
        c4_prefixes = (
        ...
        contracts = [
            ...
            if owner.startswith(c4_prefixes)
        ...
        self.assertEqual(
            settled_requirements("lib/bash/core/format_outputs.sh::format_print_cmd"),
            (
                "bash >= 4.4",
    ...
    def test_c5_scaling_contracts_distinguish_direct_and_delegated_tools(self) -> None:
        self.assertEqual(
            settled_requirements(
    ...
    def test_c5_generated_options_do_not_inherit_executed_command_requirements(self) -> None:
        self.assertEqual(
            settled_requirements("bin/submit_compute_signal.sh::set_args_opt"),
            ("bash >= 4.4",),
        )
        self.assertEqual(
            settled_requirements("bin/submit_compute_signal.sh::run_dry_or_wet"),
            (
                "bash >= 4.4",
    ...
    def test_c5_compute_signal_contract_tracks_python_environment_and_resources(self) -> None:
        self.assertEqual(
    ...
    def test_c5_dynamic_filter_dispatch_retains_complete_reachable_contract(self) -> None:
        self.assertEqual(
            settled_requirements("bin/submit_filter_alignments.sh::run_filtering"),
            (
                "bash >= 4.4",
                "awk",
                "basename",
    ...
    def test_c6_direct_python3_execution_is_literal_callable_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = Path(tmp) / "direct_python3.sh"
            source.write_text(
    ...
    def test_c6_python3_in_help_fixture_text_is_not_runtime_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = Path(tmp) / "fixture_text.sh"
            source.write_text(
...
""",
    ...
    def test_c6_python_provider_discovery_and_loopback_probe_are_settled_separately(self) -> None:
        self.assertEqual(
    ...
    def test_c6_dynamic_interpreter_contract_does_not_invent_one_spelling(self) -> None:
        self.assertEqual(
            settled_requirements("tests/support/test_helpers.sh::find_port_free"),
            ("bash >= 4.4", "Caller-supplied Python interpreter >= 3.11"),
        )
        self.assertEqual(
            settled_requirements("tests/support/test_helpers.sh::wait_http_local"),
            (
                "bash >= 4.4",
    ...
    def test_c6_tested_child_requirements_are_not_helper_requirements(self) -> None:
        expected = {
    ...
    def test_c6_optional_integration_dependencies_keep_source_conditions(self) -> None:
        self.assertEqual(
            settled_requirements(
    ...
    def test_c6_python_candidates_are_fully_dispositioned(self) -> None:
        ...
        for identity in (
            ...
            evidence = evidence_for_owner(identity, source, kind, root)
            self.assertFalse(
                [item for item in evidence if item.category == "unknown_callable"]
            )
