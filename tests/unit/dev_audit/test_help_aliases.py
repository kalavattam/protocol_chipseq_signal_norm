...
class HelpAliasesTest(unittest.TestCase):
    ...
    def test_combined_case_arm_splits_at_each_logical_short_alias(self) -> None:
...
"""
    ...
    def test_literal_parser_acceptance_controls_public_visibility(self) -> None:
    ...
    def test_help_short_alias_is_public_without_historical_help_evidence(self) -> None:
...
"""
    ...
    def test_documented_d_and_ah_short_aliases_are_public_when_accepted(self) -> None:
...
"""
    ...
    def test_top_level_parser_alias_fallback_uses_central_extractor(self) -> None:
        text = """while [[ "$#" -gt 0 ]]; do
    case "${1}" in
        -dr|--dry|--dry[_-]run)
            shift
...
"""
    ...
    def test_usage_may_omit_an_option_but_cannot_use_its_short_alias(self) -> None:
        omitted = """function demo() {
    cat << EOM
Usage
-----
  demo value
...
"""
        findings, _ = check_document(REPO_ROOT, "lib/bash/core/demo.sh", omitted)
...
----------
  --dry_run : flag
"""
        normalized = normalize_document(text, {3: ("-dr", "--dry", "--dry_run")})
...
"""
    ...
    def delegated_filter_inventory(self) -> list[dict[str, object]]:
        ...
        return [
            ...
            if row["documented_owner"]
            in {"filter_alignment_sc", "filter_alignment_sp"}
        ]
    ...
    def test_public_owner_inherits_directly_delegated_parser_contract(self) -> None:
        source = """function shared_parser() {
    case "${1}" in
        -t|--threads)
            shift
            ;;
...
"""
        ...
        self.assertIsNotNone(bindings, "delegated binding discovery API is missing")
...
"""
    ...
    def test_filter_alignment_sc_receives_only_shared_public_contract(self) -> None:
    ...
    def test_filter_alignment_sp_receives_shared_and_sp_public_contract(self) -> None:
    ...
    def test_filter_alignment_sc_rejected_mode_alias_is_an_overclaim(self) -> None:
        text = (REPO_ROOT / "lib/bash/workflows/filter_alignment.sh").read_text()
        marker = """  -cc, --chk_chr : flag
    Check chromosomes in output alignment files.
"""
        overclaim = """  -tg, --tg : flag
...
"""
    ...
    def test_delegated_hidden_pattern_aliases_are_not_public_rows(self) -> None:
    ...
    def test_unsupported_and_transposed_filter_spellings_stay_rejected(self) -> None:
        for owner in ("filter_alignment_sc", "filter_alignment_sp"):
            row = self.delegated_filter_row(owner)
    ...
    def test_delegated_usage_remains_canonical_long_only(self) -> None:
        ...
        expected = {
            "filter_alignment_sc": (
                "--help", "--threads", "--fil_in", "--fil_out",
                "--ref_fa", "--mito", "--chk_chr",
            ),
            "filter_alignment_sp": (
                "--help", "--threads", "--fil_in", "--fil_out",
                "--ref_fa", "--mito", "--tg", "--mtr", "--chk_chr",
            ),
        }
        for owner, aliases in expected.items():
            self.assertEqual(usage[owner], aliases)
            self.assertTrue(all(alias.startswith("--") for alias in usage[owner]))

    def test_internal_filter_parser_documents_options_after_delimiter(self) -> None:
        text = (REPO_ROOT / "lib/bash/workflows/filter_alignment.sh").read_text()
        rows = [
            row.aliases
        ...
        body = help_aliases.function_bodies(text)["_parse_args_filter_alignment"]
...
"""
    ...
    def test_directly_owned_parser_help_relationship_is_unchanged(self) -> None:
        text = """function demo() {
    cat << EOM
Usage
-----
...
"""
    ...
    def test_all_current_help_owners_have_zero_unexplained_alias_gaps(self) -> None:
        paths = sorted(
    ...
    def test_submit_compute_signal_csv_fragment_aliases_are_settled(self) -> None:
        ...
        parser_body = __import__("help_aliases").function_bodies(source)["parse_args"]
