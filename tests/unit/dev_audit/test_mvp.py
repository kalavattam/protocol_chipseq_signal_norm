...
from dev.audit.shell_validation import (
    MINIMUM_BASH,
    POSIX_BOOTSTRAP,
    resolve_env_bash,
    syntax_command,
...
class AuditMvpTest(unittest.TestCase):
    ...
    def make_repo(self, directory: Path) -> Path:
        ...
        subprocess.run(
            ["git", "-C", str(directory), "commit", "-qm", "initial"],
            check=True,
        )
    ...
    def baseline(self) -> dict[str, object]:
        ...
        return {
            ...
            "paths": [
                {
                    "path": f"src/file_{index:03d}.txt",
                    "initial_git_status": "??",
                }
    ...
    def test_parser_and_staged_whitespace_evidence(self) -> None:
        ...
        staged = parse_result(
            ...
            CommandResult(
                ...
                "tracked.txt:2: trailing whitespace.\n",
                "",
                2,
            ),
            ".",
    ...
    def test_private_evidence_detects_content_change(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                reader.read("private", "evidence.txt", "test"),
                "before\n",
            )
            ...
            self.assertEqual(
                reader.verify()[0]["verification_status"],
                "stale",
            )

    def test_cohort_reconciliation_preserves_renamed_and_missing_paths(
        self,
    ) -> None:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = self.make_repo(Path(temp_dir) / "public")
            (root / "current.txt").write_text("current\n", encoding="utf-8")

            baseline = {
                "authored_path_count": 2,
                "paths": [{"path": "old.txt"}, {"path": "gone.txt"}],
            }
            ledger = [
                {
                    "path": "current.txt",
                    "cohort_origin_path": "old.txt",
                    "review_status": "unreviewed",
                    "disposition": "unresolved",
                    "scope_cohort": "baseline_cleanup",
                },
            ]

            progress = cohort_progress(root, baseline, ledger)
            states = {
                row["baseline_path"]: row["state"]
                for row in progress["baseline_paths"]
            }
    ...
    def test_path_relocations_preserve_baseline_identity(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                {"tests/unit/example/test_worker.py": "dev/test_worker.py"},
            )
    ...
    def test_inventory_separates_baseline_and_audit_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "dev/audit/tool.py").write_text(
                "x = 1\n",
                encoding="utf-8",
            )
            ...
            subprocess.run(
                ["git", "-C", str(root), "add", "-f", "dev/.DS_Store"],
                check=True,
            )
            ...
            config = {
                "artifacts": {
                    "bytecode_globs": ["**/*.pyc", "*.pyc"],
                    "other_generated_globs": ["**/.DS_Store"],
                },
                "audit_infrastructure": {
                    "paths": ["dev/audit/**"],
                    "exclude": ["artifacts/dev/audit/**"],
                },
            }
            baseline = {"paths": [{"path": "tracked.txt"}]}
            rules: list[dict[str, object]] = []

            ledger, artifacts, _ = build_inventory(
                root,
                config,
                baseline,
                rules,
            ...
            self.assertEqual(
                by_path["tracked.txt"]["scope_cohort"],
                "baseline_cleanup",
            )
            self.assertEqual(
                by_path["dev/audit/tool.py"]["scope_cohort"],
                "audit_infrastructure",
            )
            self.assertEqual(
                {row["artifact_type"] for row in artifacts},
                {"generated_bytecode", "other_generated"},
            )

    def test_inventory_does_not_execute_checkers_on_deleted_later_paths(
        self,
    ) -> None:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                by_path["legacy.sh"]["content_source"],
                "index_deleted",
            )
    ...
    def test_recursive_glob_includes_direct_children(self) -> None:
        ...
        self.assertTrue(
            path_matches(
                "dev/schemas/finding.schema.json",
                [
                    "dev/schemas/**",
                ],
            ),
    ...
    def test_canonical_report_integrity_ignores_only_ds_store(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(
                ValueError,
                "canonical report integrity mismatch",
            ):
                validate_canonical_report_manifest(report, expected)
    ...
    def test_verify_reports_stale_fingerprints(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (report / "ledger.ndjson").write_text(
                json.dumps(row) + "\n",
                encoding="utf-8",
            )
            (report / "findings.ndjson").write_text(
                json.dumps(row) + "\n",
                encoding="utf-8",
            )
            (report / "prompts.ndjson").write_text(
                json.dumps(
                    {
                        "batch_id": "one",
                        "target_fingerprints": {"tracked.txt": fingerprint},
                    },
                )
                + "\n",
                encoding="utf-8",
            ...
            with (report / "staleness.ndjson").open(
                encoding="utf-8",
            ) as handle:
    ...
    def test_verify_marks_missing_artifacts_incomplete(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            summary = json.loads(
                (report / "verification.json").read_text(encoding="utf-8"),
            )
    ...
    def test_baseline_validation_rejects_requested_invariants(self) -> None:
        ...
        cases: list[tuple[str, callable]] = [
            ...
            (
                "bytecode",
                lambda value: value["paths"][0].update({"path": "cache.pyc"}),
            ),
            (
                "absolute",
            ...
            (
                "wrong_count",
                lambda value: value.update({"authored_path_count": 3}),
            ),
            (
                "malformed",
                lambda value: value["paths"].__setitem__(0, "not-an-entry"),
            ),
        ]
    ...
    def test_shell_syntax_batch_returns_per_path_subresults(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "good.sh").write_text(
                "#!/usr/bin/env bash\necho good\n",
                encoding="utf-8",
            )
            (root / "bad.sh").write_text("if then\n", encoding="utf-8")

            result, subresults = run_shell_syntax_batch(
                ["good.sh", "bad.sh"],
                root,
                10,
                4096,
            ...
            self.assertEqual(
                [row["status"] for row in subresults],
                ["completed", "finding"],
            )

    def test_authoritative_bash_satisfies_minimum_and_is_reported(
        self,
    ) -> None:
        """
    ...
    def test_syntax_routing_uses_env_bash_except_for_posix_bootstrap(
        self,
    ) -> None:
        """
