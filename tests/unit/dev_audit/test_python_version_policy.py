...
class PythonVersionPolicyTest(unittest.TestCase):
    ...
    def test_authoritative_minimum_is_python_3_11(self) -> None:
        from python_version_policy import MINIMUM_PYTHON, RULE_ID
    ...
    def test_supported_version_boundary(self) -> None:
        from python_version_policy import is_supported_version
    ...
    def test_environment_requirement_accepts_supported_newer_versions(self) -> None:
        from python_version_policy import environment_python_version
    ...
    def test_floor_parser_rejects_syntax_newer_than_python_3_11(self) -> None:
        from python_version_policy import parse_floor_syntax

        with self.assertRaises(SyntaxError):
            parse_floor_syntax("type Alias = int\n", "python312_only.py")
        parse_floor_syntax("Alias = int\n", "python311.py")

    def test_inventory_classifies_maintained_entry_points_and_modules(self) -> None:
        from python_version_policy import inventory_repository

        root = Path(__file__).resolve().parents[3]
        inventory = inventory_repository(root)
        self.assertTrue(inventory)
        self.assertTrue(any(row.role == "entry_point" for row in inventory))
        self.assertTrue(any(row.role == "imported_module" for row in inventory))
        self.assertTrue(all(row.minimum == "Python >= 3.11" for row in inventory))

    def test_guard_policy_rejects_python_3_10_and_accepts_python_3_11(self) -> None:
        from python_version_policy import guard_findings, local_guard
    ...
    def test_repository_parity_has_one_stable_rule_id(self) -> None:
        from python_version_policy import scan_repository
