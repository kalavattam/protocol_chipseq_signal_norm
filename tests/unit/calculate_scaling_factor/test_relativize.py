...
def test_format_scaled_strips_trailing_zeros():
    assert format_scaled(0.5000001, 3) == "0.5"


def test_determine_scaling_column_prefers_siq():
    assert determine_scaling_column(["sample", "siq", "spike"]) == "siq"


def test_relativize_scales_ip_and_optionally_input():
    header = ["sample", "spike"]
    rows = [
        {"sample": "IP_A", "spike": "2"},
        {"sample": "IP_B", "spike": "4"},
...
def test_relativize_rejects_missing_sample_column():
